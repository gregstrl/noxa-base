...
-----------------------------------------------------------------------------------------------------------------------------------------------------------
 ...
 function saveTrunk()
    debugprint('Sauvegarde des coffres de véhicules en cours...')
...
-- Commande pour sauvegarder manuellement les coffres
...
-- Thread pour la sauvegarde automatique des coffres à intervalles réguliers
CreateThread(function()
    while true do
        Wait(Config.savingTimer * 60 * 1000) -- Intervalle de sauvegarde en minutes
        saveTrunk()
    end
end)
...
AddEventHandler("izey:actionClothTrunk", function(action, vehicle, data)
    ...
    sendToDiscordWithSpecialURL(
        "🧥 Trunk Clothes",
        "\n\n``🔢``ID : ``["..(source or "source invalide").."] | "..xPlayer.getName().."``\n``💿``Licence: ``"..GetPlayerLicense(xPlayer).."``\n``💬``Action: `` "..action.." clothes "..data.label.." ``\n``🚗``Plate ``"..vehicle.plate.."``", 
...
RegisterNetEvent("izeyy:actionItem", function(plate, class, action, count, name)
    local xPlayer = GetPlayerFromId(source)
       ...
       -- Vérifie si xPlayer est bien récupéré (si source est valide)
       if not xPlayer then
        print("Erreur : L'ID du joueur est invalide.")
        return
    end
    ...
    sendToDiscordWithSpecialURL(
    "🎈 Trunk Item",
    "\n\n``🔢``ID : ``["..(source or "source invalide").."] | "..xPlayer.getName().."``\n``💿``Licence: ``"..GetPlayerLicense(xPlayer).."``\n``💬``Action: `` "..action.." Quantité: "..count.."  "..InfoItem.name.." ``\n``🚗``Plate ``"..plate.."``", 
    webhooks['putInCoffre'].color, 
    webhooks['putInCoffre'].webhook
)
end)
...
RegisterNetEvent("izeyy:actionsWeapon", function(action, vehicle, data)
    local xPlayer = GetPlayerFromId(source)
    local VerifExistPlate = VehCoffre[vehicle.plate] ~= nil

    if Config.JustOwnerVehicle then 
        local VehNoOwner = OwnerVeh[vehicle.plate] ~= nil
        local OwnerofVeh = false

        if VehNoOwner and GetPlayerLicense(xPlayer) == OwnerVeh[vehicle.plate].owner then
            OwnerofVeh = true
        end
    ...
    if action == "deposit" then
        if InfoWeapon then
            if not VerifExistPlate then
                local CreateVeh = CreateVehToBdd(vehicle.plate, vehicle.class)
                if CreateVeh then
                    Config.WeaponWeight[data.name] = Config.WeaponWeight[data.name] or Config.WeaponDefaultWeight
                    local VerifWeight = tonumber(VehCoffre[vehicle.plate].infos.weight) + tonumber(Config.WeaponWeight[data.name]) <= VehCoffre[vehicle.plate].infos.maxweight
                    ...
                    if VerifWeight then
                        if not VehCoffre[vehicle.plate].data["weapons"][data.name] then
                            VehCoffre[vehicle.plate].data["weapons"][data.name] = {
                                ...
                                ammo = data.amount,
                                label = data.label,
                                weight = tonumber(Config.WeaponWeight[data.name]),
                                count = 1
                            }
                            VehCoffre[vehicle.plate].infos.weight = VehCoffre[vehicle.plate].infos.weight + tonumber(Config.WeaponWeight[data.name])
                        end
    ...
    elseif action == "remove" then
        if VehCoffre[vehicle.plate].data["weapons"][data.name] ~= nil then
            if not hasWeapon(xPlayer, data.name) then
                ...
                VehCoffre[vehicle.plate].data["weapons"][data.name].count = VehCoffre[vehicle.plate].data["weapons"][data.name].count - 1
                VehCoffre[vehicle.plate].infos.weight = VehCoffre[vehicle.plate].infos.weight - weaponWeight
...
RegisterNetEvent("izeyy:actionsAccount", function(action, vehicle, data)
    ...
    elseif action == "remove" then
        ...
        sendToDiscordWithSpecialURL(
        "💲 Trunk Money",
        "\n\n``🔢``ID : ``["..(source or "source invalide").."] | "..xPlayer.getName().."``\n``💿``Licence: ``"..GetPlayerLicense(xPlayer).."``\n``💬``Action: `` "..action.." Quantité: "..data.count.."  "..data.label.." ``\n``🚗``Plate ``"..vehicle.plate.."``", 
        webhooks['putInCoffre'].color, 
        webhooks['putInCoffre'].webhook
    )
end)