...
RegisterNUICallback("PutIntoTrunk", function(data, cb)
    if IsPedRagdoll(PlayerPedId()) then
        ESX.ShowNotification("Impossible de réaliser cette action")
        return
    end
        ...
        if not IsPedSittingInAnyVehicle(PlayerPedId()) then 
            ...
            if distance ~= -1 and distance <= 3.0 then
                ...
                if locked == 1 then
                    ...
                    ESX.ShowNotification("Impossible le véhicule est verrouillé")

                end
            else
                ESX.ShowNotification("Il n\'y a pas de véhicule près de vous.")

            end
        else
            ESX.ShowNotification("Vous ne pouvez pas faire ça dans un véhicule.")
        end
