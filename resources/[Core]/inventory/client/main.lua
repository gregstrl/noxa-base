...
function openInventory()
    if IsPlayerDead() then
        ESX.ShowNotification("~r~Vous ne pouvez pas ouvrir l'inventaire en étant mort.")
        return
    end
...
-----------------------------------------------------------------------------------------------------------------------------------------------------------
 ...
 RegisterCommand('enleverpedinv', function()
     removePedInv(true)
 end)
...
function removePedInv(bool)
    Inv.noPedInv = GetFieldValueFromName('inventory').pedscreen
    if Inv.noPedInv == false or  Inv.noPedInv == nil then
        Inv.noPedInv = true
        SetFieldValueFromNameEncode('inventory', {pedscreen = bool})
    else
        if Inv.noPedInv == true then
            Inv.noPedInv = false
            SetFieldValueFromNameEncode('inventory', {pedscreen = bool})
...
function CreatePedScreen(firstTime)
    ...
    if pedScreen then
        exports['modules']:PedScreenDelete(pedScreen)
        pedScreen = nil
    end
    pedScreen = exports['modules']:PedScreenCreate({
...
function RefreshPedScreen()
    if Inv.isInInventory or Inv.openInvPlayer or Inv.isInTrunk or Inv.isInProperty then
        if pedScreen then
            exports['modules']:PedScreenDelete(pedScreen)
        end
        pedScreen = exports['modules']:PedScreenCreate({
...
Inv.FastWeapons = GetFieldValueFromName('inventory').name and GetFieldValueFromName('inventory').name or {}
        ...
        if IsPedRagdoll(PlayerPedId()) then
            ...
            elseif data.item.type == "item_weapon" then 
                ...
                if not weaponLock then
                    ...
                    if  weaponEquiped ~= data.item.name then
                        ...
                        Wait(150)
                        TriggerEvent('Framework:autoAmmo', data.item.name)
                        weaponLock = false
                    else 
                        weaponEquiped = nil
            ...
            elseif data.item.type == "item_idcard" then
                ...
                if closestPlayer ~= -1 and closestDistance < 2.5 then
                    TriggerServerEvent('jsfour-idcard:open', GetPlayerServerId(closestPlayer), data.item.value, data.item.name)
                else
                    SendNUIMessage({
                        action = "open:MenuIdCard",
...
RegisterNUICallback('giveItem', function(data)
    if not Inv.isInTrunk then
        ...
        if closestPlayer ~= -1 and closestDistance < 2.5 then
            ...
            local itemLabel = data.item.label or "Objet inconnu"
                    ...
                    KeyboardUtils.use(Locales[Config.Language]['quantite'], function(result)
                        if result ~= nil then
                            ...
                            if quantity and quantity > 0 then
                                ESX.TriggerServerCallback('izey:checkItemQuantity', function(hasItem)
                                    if hasItem then
                                        ...
                                        TriggerServerEvent('izey:giveItem', targetPlayerId, data.item.name, quantity, "item_standard")
                                        TriggerServerEvent('interact:sendLogsGive', data.item.name, quantity, targetPlayerId)
                                        loadPlayerInventory('item', nil, true, true)

                                        -- Ajout du /me
                                        ExecuteCommand("me donne " .. quantity .. "x " .. itemLabel .. " à un joueur proche.")
                                    else
                                        NotificationInInventory(Locales[Config.Language]['not_enough_item'], 'error')
                                    end
                                end, data.item.name, quantity)
                            end
                        end
                    end)
                else
            ...
            elseif data.item.type == "item_weapon" then
                if not Config.WeaponNoGive[data.item.name] then
                    ESX.TriggerServerCallback('izey:checkWeaponOwnership', function(hasWeapon)
                        if hasWeapon then
                            function_inv:RequestAnimDict("mp_common", function()
                                TaskPlayAnim(playerPed, "mp_common", "givetake2_a", 2.0, -2.0, 2500, 49, 0, false, false, false)
                            end)
                            TriggerServerEvent('izey:giveItem', targetPlayerId, data.item.name, 255, "item_weapon", itemLabel)
                            TriggerServerEvent('interact:sendLogsGiveWeapon', itemLabel, targetPlayerId)
                            loadPlayerInventory('item', nil, true, true)

                            ExecuteCommand("me donne une arme : " .. itemLabel .. " à un joueur proche.")
                        else
                            NotificationInInventory(Locales[Config.Language]['not_enough_weapon'], 'error')
                        end
                    end, data.item.name)
                else
                ...
                KeyboardUtils.use(Locales[Config.Language]['quantite'], function(result)
                    if result ~= nil then
                        ...
                        if amount and amount > 0 then
                            ESX.TriggerServerCallback('izey:checkAccount', function(hasAccount)
                                if hasAccount then
                                    function_inv:RequestAnimDict("mp_common", function()
                                        TaskPlayAnim(playerPed, "mp_common", "givetake2_a", 2.0, -2.0, 2500, 49, 0, false, false, false)
                                    end)
                                    TriggerServerEvent('izey:giveItem', targetPlayerId, data.item.name, amount, "item_account", itemLabel)
                                    TriggerServerEvent('interact:sendLogsGiveAccount', itemLabel, targetPlayerId)
                                    loadPlayerInventory('item', nil, true, true)

                                    ExecuteCommand("me donne " .. amount .. "$ de " .. itemLabel .. " à un joueur proche.")
                                else
                                    NotificationInInventory(Locales[Config.Language]['not_enough_account'], 'error')
                                end
                            end, data.item.name, amount)
                        else
                            NotificationInInventory(Locales[Config.Language]['invalid_amount'], 'error')
                        end
                    end
                end)
