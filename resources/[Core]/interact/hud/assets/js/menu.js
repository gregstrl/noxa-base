$(document).ready(function () {
    ...
    window.addEventListener('message', function (event) {
        ...
        */
        ...
        if (event.data.type === "login") {
            ...
            document.getElementById("hud").style.backgroundColor = "#0055ff5e";
        ...
        $(".button").on("click", function () {
            ...
            $.post('http://alyniahud/disablenuifocus', JSON.stringify({
                nuifocus: false
            }));
        });
        ...
        if (event.data.menu == 'user') {
            ...
            buttons.forEach(button => {
                ...
                $(`.${button.name}`).on('click', function (e) {
                    e.preventDefault();
                    $.post(`http://alyniahud/${button.event}`, JSON.stringify({
                        id: idEnt
                    }));
                    // $(this).find('.text').text($(this).find('.text').text() == 'Ouvrir le coffre' ? 'Fermer le coffre' : 'Ouvrir le coffre');
                });
    ...
    $('.openCoffre').on('click', function (e) {
        e.preventDefault();
        $.post('http://alyniahud/togglecoffre', JSON.stringify({
            id: idEnt
        }));
        $(this).find('.text').text($(this).find('.text').text() == 'Ouvrir le coffre' ? 'Fermer le coffre' : 'Ouvrir le coffre');
    });
    ...
    $('.openCapot').on('click', function (e) {
        e.preventDefault();
        $.post('http://alyniahud/togglecapot', JSON.stringify({
            id: idEnt
        }));
    ...
    $('.lock').on('click', function (e) {
        e.preventDefault();
        $.post('http://alyniahud/togglelock', JSON.stringify({
            id: idEnt
        }));
    ...
    $('.cheer').on('click', function (e) {
        e.preventDefault();
        $.post('http://alyniahud/cheer', JSON.stringify({
            id: idEnt
        }));
    });
