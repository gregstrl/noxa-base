...
function RageUIafk.Separator(Label)
    ...
    if CurrentMenu ~= nil then
        if CurrentMenu() then
            ...
            if CurrentMenu.Pagination.Minimum <= Option and CurrentMenu.Pagination.Maximum >= Option then
                if (Label ~= nil) then
                    RenderText(Label, CurrentMenu.X + SettingsButton.Text.X + (CurrentMenu.WidthOffset * 2.5 ~= 0 and CurrentMenu.WidthOffset * 2.5 or 200), CurrentMenu.Y + SettingsButton.Text.Y + CurrentMenu.SubtitleHeight + RageUIafk.ItemOffset, 0, SettingsButton.Text.Scale - 0.05, 245, 245, 245, 255, 1)
                end
