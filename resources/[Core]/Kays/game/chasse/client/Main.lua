...
AddEventHandler("Chasse:returnStart", function(chasse)
    ...
    ESX.ShowNotification("Bonne chasse ! Je t'ai mis un fusil dans ton sac et un véhicule proche de toi")
    local hash = GetHashKey("rebel2")
    if not ESX.Game.IsSpawnPointClear(chasse.spawnVeh, 5.0) then
        ESX.ShowNotification("Impossible de faire apparaître le véhicule")
    else
    ...
    while not animalDecoupe do
        ...
        if #(GetEntityCoords(PlayerPedId())-entityCoords) < 1.5 then
            ...
            if IsControlJustReleased(0, 38) then
                ...
                ESX.ShowNotification("[~g~Succès~s~] Vous avez réussi à découper l'animal")
...
AddEventHandler("Chasse:returnStop", function(chasse, message)
    ...
    if message then
        ...
        ESX.ShowNotification("Bon retour parmis nous, j'ai pris ton fusil inutile de le chercher")
    else
