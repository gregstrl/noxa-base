...
local actions = {
    ["Harvest"] = function(source, drug, xPlayer)
        ...
        if xPlayer.canCarryItem(harvestItem, 1) then
            ...
            TriggerClientEvent('esx:showNotification', source, '~b~Vous n\'avez pas assez de place dans votre inventaire')
        end
    end,
    ...
    ["Sell"] = function(source, drug, xPlayer)
        ...
        elseif sale == 1 then
            ...
            if (
                not xPlayer.job2.name == "yadkuza" 
                or not xPlayer.job2.name == "maddrazo" 
                or not xPlayer.job2.name == "madrabunta" 
                or not xPlayer.job2.name == "mafdiamarocaine"
                or not xPlayer.job2.name == "bldoods"
                or not xPlayer.job2.name == "famildies"
            ) 
            then
...
AddEventHandler("exedrugs_deletedrug", function(drugID)
    ...
    if not drugs[drugID] then
        TriggerClientEvent("esx:showNotification", source, "~b~Cette drogue n'existe plus")
        return
    end
...
AddEventHandler("exedrugs_create", function(builderInfos)
    ...
    MySQL.Async.fetchAll("SELECT * FROM items", {}, function(items)
       ...
       -- if not serverItems[builderInfos.rawItem] or not serverItems[builderInfos.treatedItem] then
          --  TriggerClientEvent("esx:showNotification", source, "~b~Un des item du drugsbuilder est invalide, création abandonnée")
          --  return
      --  end
