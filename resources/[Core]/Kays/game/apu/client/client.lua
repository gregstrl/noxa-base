...
AddEventHandler("initializeApuBlip", function(houseIndex,duration)
    ...
    ESX.ShowNotification("[~r~Alerte~s~] Un civil a appellé la police à cause d'un possible cambriolage.")
                    ...
                    while true do
                        ...
                        if DoesEntityExist(bag) then
                            if GetDistanceBetweenCoords(GetEntityCoords(PlayerPedId()), GetEntityCoords(bag), true) <= 1.5 then
                                wait = 1
                                DrawText3D(GetEntityCoords(bag).x, GetEntityCoords(bag).y, GetEntityCoords(bag).z, "Appuyez sur ~g~E~s~ pour ramasser le sac.", 12)
...
local function MenaceApuMore(apu)
    ESX.ShowNotification("Vous devez menacer ~r~APU~s~.")
...
AddEventHandler("initializePoliceBlip2", function(ApuIndex)
    PlaySoundFrontend(-1, "Enemy_Deliver", "HUD_FRONTEND_MP_COLLECTABLE_SOUNDS", 1)
    ESX.ShowNotification("[~r~Alerte~s~] Un civil a appellé la police à cause d'un braquage de supérette.")
                        ...
                        if not eSup.IsRobbing then
                            ...
                            if canRob == true then
                                ...
                                if chance >= 0 and chance <= 25 then
                                    ...
                                    eSup.BarUpdate = AddTimerBar("Menace :",{percentage = 0.0, bg = {0, 255, 0}, fg = {247,247,247,255}})
                            ...
                            elseif canRob == 'no_cops' then
                                ...
                                while wait >= GetGameTimer() do
                                    Wait(0)
                                    ESX.ShowNotification('[~r~Impossible~s~] Manque de policier en ville.')
                                    
                                    SetFakeWantedLevel(0)
                                end
                            else
                                ESX.ShowNotification("[~r~Impossible~s~] Cette superette a déjà été braqué, le vendeur n'a plus rien.")
