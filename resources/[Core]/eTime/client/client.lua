...
Citizen.CreateThread(function()
    if Config.Framework == 'esx' then
        ...
        AddEventHandler(Config.FrameworkTriggers.load, function(xPlayer)
            ...
            Wait(3000)
            TriggerServerEvent('eTime:SyncMe', {time = true, weather = true})
        end)

        RegisterNetEvent('vSync:toggle')
        AddEventHandler('vSync:toggle', function(boolean)
            TriggerEvent('eTime:PauseSync', boolean)
        end)
...
RegisterNetEvent('eTime:PauseSync')
AddEventHandler('eTime:PauseSync', function(boolean, time)
    if boolean then
        ...
        PauseSync.state = false
        TriggerServerEvent('eTime:SyncMe')
    end
end)

RegisterNetEvent('eTime:ForceUpdate')
AddEventHandler('eTime:ForceUpdate', function(data)
    ...
    if data.tsunami ~= nil and Config.TsunamiWarning and self.tsunami ~= data.tsunami then
        ...
        TriggerEvent('eTime:StartTsunamiCountdown', data.tsunami)
    end
end)

RegisterNetEvent('eTime:SyncWeather')
AddEventHandler('eTime:SyncWeather', function(data)
    if not PauseSync.state then
        CheckSnowSync(data.weather)
        self.weather = data.weather
        ChangeWeather(self.weather, data.instantweather)
...
RegisterNetEvent('eTime:SyncTime')
AddEventHandler('eTime:SyncTime', function(data)
    if not PauseSync.state then
...
RegisterNUICallback('instanttime', function(data)
    TriggerServerEvent('eTime:ToggleInstantChange:Time', data.instanttime)
end)

RegisterNUICallback('instantweather', function(data)
    TriggerServerEvent('eTime:ToggleInstantChange:Weather', data.instantweather)
end)

RegisterNUICallback('change', function(data)
    NUI_status = false
    TriggerServerEvent('eTime:ForceUpdate', data.values)
    if data.savesettings then
        --print('Saving Settings - please wait 2 seconds ...')
        Wait(2000)
        TriggerServerEvent('eTime:SaveSettings')
...
RegisterNetEvent('eTime:OpenUI')
AddEventHandler('eTime:OpenUI', function(values)
    TriggerEvent('eTime:ToggleNUIFocus')
    SendNUIMessage({action = 'open', values = values})
end)

RegisterNetEvent('eTime:ToggleNUIFocus')
AddEventHandler('eTime:ToggleNUIFocus', function()
    NUI_status = true
    while NUI_status do
        Wait(0)
...
RegisterNetEvent('eTime:StartTsunamiCountdown')
AddEventHandler('eTime:StartTsunamiCountdown', function(boolean)
