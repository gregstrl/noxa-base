...
AddEventHandler('esx:playerLoaded', function(source)
            ...
            if tonumber(result) ~= 157 then
                if tonumber(result) <= os.time() then
                    ...
                    if account.vip == 3 then
                        KoyServerUtils.toClient('KoyVIP:updateVip', source, 1)
                        ...
                        KoyServerUtils.toClient('KoyVIP:updateVip', source, 0)
                        MySQL.Async.execute('UPDATE tebex_accounts SET `vip` = @vip, `expiration` = @expiration WHERE steam = @steam', {
                            ['@steam'] = steam,
                            ['@vip'] = 0,
                            ['@expiration'] = 0,
                    ...
                    xPlayer.showNotification("Votre VIP : ["..labeltext.."~s~]\nPrendra fin dans ~b~"..math.floor(day).. "~s~ jours, ~g~"..txthrs.."~s~ heure(s).")
                    KoyServerUtils.toClient('KoyVIP:updateVip', source, account.vip)
                end
            else
                xPlayer.showNotification("Votre VIP est Permanent")
                KoyServerUtils.toClient('KoyVIP:updateVip', source, account.vip)
            end
        end)
    else
    ...
    KoyServerUtils.toClient('KoyVIP:updatePoints', source, account.points)
end)
Citizen.CreateThread(function()
    while true do
        -- Attendre 60 secondes entre chaque itération
        Wait(60000)
        ...
        if TICK.SHARE_ACCOUNTS < timer then
...
function Tebex:tickShareAccounts()
    ...
    TriggerEvent('KoyVIP:shareAccounts', data)
    KoyServerUtils.toClient('KoyVIP:updatePlayerKoyVIP', -1, data)
        ...
        Switch(command, {
            ['addVip'] = function()
                ...
                MySQL.Async.execute('UPDATE tebex_accounts SET `vip` = @vip, `expiration` = @expiration WHERE steam = @steam', {
                    ...
                    account.vip = rank
                    if source ~= nil then
                        KoyServerUtils.toClient('KoyVIP:updateVip', source, rank)
                    end
                end)
            end,
...
RegisterCommand("addVip", function(src, args)
    ...
    if (isAllowed) then
        ...
        if (target) then
            ...
            target.showNotification("Vous avez recu le "..ranks[data["rank"]].name.."")

            if (xPlayer) then
                xPlayer.showNotification("Vous avez donner un "..ranks[data["rank"]].name.."")
            else
                ESX.Logs.Success(("Vous avez donner un ~b~%s"):format(ranks[data["rank"]].name));
            end

            ACCOUNTS[license].vip = data["rank"];
            KoyServerUtils.toClient('KoyVIP:updateVip', target.source, vipRank);
        ...
        if (xPlayer) then
            xPlayer.showNotification("[~r~Impossible~h~] Vous n'avez pas la permission de faire cela")
        end
...
--             ['@transaction'] = transaction
--         })
--         if vipRank == 1 then vipLabel = "VIP ~y~Gold~s~." money = 1000000 elseif vipRank == 2 then vipLabel = "VIP ~s~Diamond~s~." money = 3000000 end
...
--                 KoyServerUtils.toClient('KoyVIP:updateVip', xPlayer.source, vipRank)
--                 online = true
--             else
--                 online = false
--             end
...
RegisterCommand('removeVip', function(source, args)
    ...
    Citizen.CreateThread(function()
        ...
        for i=1, #xPlayers, 1 do
            ...
            if fivem == fivemPlayer then
                local xPlayer = ESX.GetPlayerFromId(xPlayers[i])
                TriggerClientEvent("esx:showNotification", xPlayer.source, "~s~Votre VIP a expiré !");
                KoyServerUtils.toClient('KoyVIP:updateVip', xPlayer.source, 0);
            end
        end
    end)
end)
...
RegisterCommand('viptime', function(source)
    ...
    if account.vip ~= 0 then
        local labeltext = 'Aucun'
        if account.vip == 1 then
            labeltext = '~b~Mini-VIP'
        elseif account.vip == 2 then
            labeltext = '~y~VIP Gold'
        elseif account.vip == 3 then
            labeltext = '~b~VIP Diamond'
        end
        MySQL.Async.fetchScalar('SELECT expiration FROM tebex_accounts WHERE steam = @steam', {
            ['@steam'] = steam
        }, function(result)
            if tonumber(result) ~= 157 then
                if tonumber(result) <= os.time() then
                    ...
                    local txthrs     = math.floor(hrs)
                    local txtminutes = math.ceil(minutes)
                    xPlayer.showNotification('Votre VIP : ['..labeltext..'~s~]\nPrendra fin dans ~g~'..math.floor(day).. '~s~ jours, ~g~'..txthrs.."~s~ heure(s).")
                end
            else
                xPlayer.showNotification('Votre VIP : [~s~'..labeltext..']~s~\nPrendra fin ~s~Jamais')
            end
        end)
    else
