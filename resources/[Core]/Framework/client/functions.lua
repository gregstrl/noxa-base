...
function ESX.ShowNotification(msg, flash, saveToBrief, hudColorIndex, title, subject, icon)
	if title == nil then title = "Wise" end
	if subject == nil then subject = "Notification" end
	if icon == nil then icon = "CHAR_LOGO" end

	TriggerEvent("RNotification:showAdvancedNotification", title, subject, msg, icon, duration or 10, location)
end
...
function ESX.ShowHelpNotification(msg, thisFrame, beep, duration)
    local location = "left"
    TriggerEvent("RNotification:showHelpNotification", msg, duration or 1, location)
end
...
function ESX.UI.Menu.Open(type, nameKay, name, data, submit, cancel, change, close)
	local menu = {}
	...
	function menu.close()
		if menu.type == 'default' then
			ESX.UI.Menu.RegisteredTypes[menu.type].close(menu.nameKay, menu.name)
		else
			ESX.UI.Menu.RegisteredTypes[menu.type].close(menu.nameKay, menu.name)

			for i = 1, #ESX.UI.Menu.Opened, 1 do
				if ESX.UI.Menu.Opened[i] then
					if ESX.UI.Menu.Opened[i].type == menu.type and ESX.UI.Menu.Opened[i].nameKay == menu.nameKay and ESX.UI.Menu.Opened[i].name == menu.name then
	...
	function menu.update(query, newData)
		if menu.type == 'default' then
			ESX.UI.Menu.RegisteredTypes[menu.type].update(menu.nameKay, menu.name, query, newData)
		else
	...
	function menu.refresh()
		ESX.UI.Menu.RegisteredTypes[menu.type].open(menu.nameKay, menu.name, menu.data, menu.submit, menu.cancel, menu.change)
	end
...
function ESX.UI.Menu.Close(type, nameKay, name)
	for i = 1, #ESX.UI.Menu.Opened, 1 do
		if ESX.UI.Menu.Opened[i] then
			if ESX.UI.Menu.Opened[i].type == type and ESX.UI.Menu.Opened[i].nameKay == nameKay and ESX.UI.Menu.Opened[i].name == name then
...
function ESX.UI.Menu.GetOpened(type, nameKay, name)
	for i = 1, #ESX.UI.Menu.Opened, 1 do
		if ESX.UI.Menu.Opened[i] then
			if ESX.UI.Menu.Opened[i].type == type and ESX.UI.Menu.Opened[i].nameKay == nameKay and ESX.UI.Menu.Opened[i].name == name then
...
function ESX.UI.Menu.IsOpen(type, nameKay, name)
	return ESX.UI.Menu.GetOpened(type, nameKay, name) ~= nil
end
...
function ESX.OpenSocietyMenu(societyName)
	exports["Koy"]:OpenSocietyMenu(societyName);
end

---@param societyName string
function ESX.OpenSocietyChest(societyName)
	exports["Koy"]:OpenSocietyChest(societyName);
end

---@param societyName string
---@param plate string
function ESX.ParkSocietyVehicle(societyName, plate)
	exports["Koy"]:ParkSocietyVehicle(societyName, plate);
end

---@param societyName string
---@param coords vector3
function ESX.OpenSocietyGarage(societyName, coords)
	exports["Koy"]:OpenSocietyGarage(societyName, coords);
end

---@return boolean
function ESX.IsPlayerHurt()
	return exports["Gamemode"]:IsHurt();
end