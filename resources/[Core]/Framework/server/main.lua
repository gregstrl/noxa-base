...
AddEventHandler('esx:playerLoaded', function(source)
    ...
    MySQL.Async.fetchAll('SELECT * FROM tebex_players_wallet WHERE identifiers = @fivemId', {
        ['@fivemId'] = fivemId
    }, function(result)
        if #result > 0 then
            xPlayer.showNotification("[~s~Boutique~s~] Vous avez déjà reçu votre cadeau de bienvenue.")
        else
            MySQL.Async.fetchAll('SELECT shopgift FROM account_info WHERE fivem = @fivem', {
                ['@fivem'] = fivemId
            }, function(result)
                if #result > 0 and result[1].shopgift == 0 then
                    MySQL.Async.fetchAll('SELECT * FROM account_info WHERE shopgift = 1 AND fivem = @fivem', {
                        ['@fivem'] = fivemId
                    }, function(existing)
                        if #existing == 0 then
                            if fivemId then
                                if #fivemId >= 7 then
                                    newfivemId = fivemId:sub(7)
                                    ...
                                    MySQL.Async.execute('UPDATE account_info SET shopgift = @shopgift WHERE fivem = @fivem', {
                                        ['@shopgift'] = 1,
                                        ['@fivem'] = fivemId
                                    })
                                    xPlayer.showNotification("[~s~Boutique~s~] Vous avez reçu votre cadeau de bienvenue dans notre boutique F1")
                                else
                                    xPlayer.showNotification("L'ID FiveM est incorrect.")
                                end
                            else
                                xPlayer.showNotification("[~s~Boutique~s~] Vous devez lier votre compte FiveM pour recevoir votre cadeau.")
                            end
                        end
                    end)
                end
	...
	table.insert(tasks, function(cb)
		MySQL.Async.fetchAll('SELECT character_id, accounts, job, job_grade, job2, job2_grade, inventory, loadout, position, xp, firstname, lastname, ammo, isDead, IsHurt FROM users WHERE identifier = @identifier', {
			...
			if not ESX.DoesJobExist(job, grade) then
				--print(('ignore job invalid for %s [job: %s, grade: %s]'):format(identifier, job, grade))
				job, grade = 'unemployed', '0'
			end

			if not ESX.DoesJobExist(job2, grade2) then
				--print(('ignore job2 invalid for %s [job: %s, grade: %s]'):format(identifier, job2, grade2))
...
AddEventHandler('esx:giveInventoryItem', function(target, type, itemName, itemCount)
	...
	if type == 'item_standard' then
		...
		if itemCount > 0 and sourceItem.count >= itemCount then
			if targetXPlayer.canCarryItem(itemName, itemCount) then
				...
				sourceXPlayer.showNotification(_U('gave_item', itemCount, ESX.Items[itemName].label, targetXPlayer.name))
				targetXPlayer.showNotification(_U('received_item', itemCount, ESX.Items[itemName].label, sourceXPlayer.name))
	...
	elseif type == 'item_account' then
		if itemCount > 0 and sourceXPlayer.getAccount(itemName).money >= itemCount then
			...
			sourceXPlayer.showNotification( _U('gave_account_money', ESX.Math.GroupDigits(itemCount), accountLabel, targetXPlayer.name), 'CHAR_KIRINSPECTEUR', 9)
			SendLogs("Argent", "Koy | Argent", "Le joueur **"..sourceXPlayer.name.."** (***"..sourceXPlayer.identifier.."***) a donner "..itemCount.." "..itemName.." à **"..targetXPlayer.name.."** (***"..targetXPlayer.identifier.."***)", "https://discord.com/api/webhooks/1258840744484012062/2AL2H_vGMUi1nwFZsv4NIeYkC910SXzPcCsCyPS-ikkwK9tbjMoQrK9BZKzfR9SVlqJl")
			targetXPlayer.showNotification( _U('received_account_money', ESX.Math.GroupDigits(itemCount), accountLabel, sourceXPlayer.name), 'CHAR_KIRINSPECTEUR', 9)

			TriggerEvent("esx:giveaccountalert", sourceXPlayer.name, targetXPlayer.name, itemName, itemCount)
		else
			sourceXPlayer.showNotification( _U('imp_invalid_amount'), 'CHAR_KIRINSPECTEUR', 9)
		end
	...
	if type == 'item_standard' then
		if itemCount == nil or itemCount < 1 then
			...
			if (itemCount > xItem.count or xItem.count < 1) then
				...
				xPlayer.showNotification(_U('threw_standard', itemCount, ESX.Items[itemName].label))
			end
		end
	elseif type == 'item_account' then
		if itemCount == nil or itemCount < 1 then
			xPlayer.showNotification(_U('imp_invalid_amount'), 'CHAR_KIRINSPECTEUR', 9)
		else
			...
			if (itemCount > account.money or account.money < 1) then
				...
				xPlayer.showNotification(_U('threw_account', ESX.Math.GroupDigits(itemCount), string.lower(accountLabel)), 'CHAR_KIRINSPECTEUR', 9)
			end
		end
	elseif type == 'item_weapon' then
		itemName = string.upper(itemName)
			...
			for i = 1, #players do
				...
				if (xPlayer) then
					...
					if (not hasBeenPromt) then
						...
						if (bank) then
							if (bank.money <= -15000) then
								xPlayer.set('hasBeenPromt', true);
								xPlayer.showNotification("Vous êtes endetté, vous devez aller régler vos dettes avant d'être débité de 2500~g~$~s~.\nDécouvert autorisé: -15000~g~$~s~\nDécouvert actuel: " .. bank.money .. "~g~$~s~");
							end
						end
					else
						...
						if (bank) then
							if (bank.money <= -15000) then
								...
								if (cash) then
									if (cash.money >= 2500) then
										xPlayer.removeAccountMoney("cash", 2500);
										xPlayer.showNotification("Vous avez été débiter de ~s~-2500$~s~ par la banque suite à vos ~s~dettes~s~.");
									end
								end
							end
						end
...
RegisterNetEvent("Koy:updateWeaponAmmo", function(weaponName, ammo)
