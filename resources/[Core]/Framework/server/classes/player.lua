...
function CreatePlayer(source, identifier, userData)
	...
	self.onRevive = function(isHospitalRespawn)
		if isHospitalRespawn then
			self.updateVIP(function(vip)
				if vip == 0 then
					...
					self.showNotification("En tant que ~b~VIP~s~, vous conservez toutes vos armes et munitions !")
				end
			end)
		end
	end
	...
	self.kick = function(reason)
		DropPlayer(self.source, ("\n\nVous avez été kick :\n\n%s\n\ndiscord.gg/wisefa"):format(reason))
	end
	...
	self.getGroup = function()
		return exports["Koy"]:GetPlayerGroup(self) or "user"
	end

	self.setGroup = function(groupName)
		local lastGroup = self.getGroup();

		if exports["Koy"]:GetGroup(groupName) ~= nil then

			exports["Koy"]:AddPlayerToGroup(self, groupName)
	...
	self.getLevel = function()
		...
		return (player_group ~= nil and exports["Koy"]:GetGroup(player_group) ~= nil and exports["Koy"]:GetGroup(player_group).level or 0)
	end
	...
	self.canCarryItem = function(name, count)
		if not ESX.Items[name] then
			print("Erreur : L'item " .. name .. " n'existe pas dans ESX.Items")
			return false
		end
	...
	self.ban = function(time, message)
        ...
        ExecuteCommand(('ban %s %s Kays is better than you %s'):format( self.source, time, message));
	end
