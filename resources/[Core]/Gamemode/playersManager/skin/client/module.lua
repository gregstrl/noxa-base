...
function LoadDefaultModel(malePed, cb)
    local playerPed = PlayerPedId()
    local characterModel = malePed and `mp_m_freemode_01` or `mp_f_freemode_01`

    if not IsModelInCdimage(characterModel) or not IsModelValid(characterModel) then
        print("Model invalide ou introuvable.")
        return
    end
...
AddEventHandler('skinchanger:loadSkin', function(skin, cb)
	...
	if (bypass ~= true and (exports["Koy"]:IsInStaffMode() and not exports["Koy"]:PlayerHasPermission("player_use_personnal_ped"))) then
		return ESX.ShowNotification("Vous ne pouvez pas faire cela en mode staff")
	end
