                            ...
                            if v.table[1] ~= nil then
                                RageUI.List(v.label, v.table, i[v.name], nil, {}, true, {
                                    onSelected = function()
                                        ...
                                        FreezeEntityPosition(PlayerPedId(), false)
                                        ESX.ShowNotification("Votre personnage à été sauvegarder avec succès !")
                                    end,
                                    onListChange = function(index, item)
                                        if index ~= nil then
                                            refreshDataSkin()
...
AddEventHandler('playerSpawned', function(_, firstSpawn)
    if (firstSpawn) then
                ...
                ESX.TriggerServerCallback('esx_skin:getPlayerSkin', function(skin, jobSkin)
                    ...
                    if (skin == nil) then
                        TriggerEvent('skinchanger:loadSkin', { sex = 0 }, function()
                            TriggerEvent('Koy:playerShouldDie');
                        end);
                    else
