	...
	if open then 
		...
		while open do 
		   RageUI.IsVisible(unicornMain2,function() 
			...
			RageUI.Checkbox("Entreprise Ouverte", nil, Society.List["unicorn"].state, {}, {
				onChecked = function()
					TriggerServerEvent('entreprise:setStatus', "unicorn", true)
					TriggerServerEvent('Ouvre:unicorn')
				end,
				onUnChecked = function()
					TriggerServerEvent('entreprise:setStatus', "unicorn", false)
			...
			RageUI.Button("Annonce ~o~[Personnaliser]", nil, {RightLabel = "→"}, true , {
				onSelected = function()
					...
					if input and input[1] then
						TriggerServerEvent('Personnaliser:unicorn', input[1])
					end
				end
			})
			RageUI.Line()
			...
			RageUI.Button("Faire une ~o~Facture", nil, {RightLabel = "→"}, true , {
                onSelected = function()
                    local montant = KeyboardInputUnicorn("Montant:", 'Indiquez un montant', '', 6)
					local amount = 0;
		...
		if player ~= -1 and distance <= 3.0 then
		  ...
		  if amount == nil then
			...
			Wait(5000)
		 	...
		 	ESX.ShowNotification("[~r~Impossible~h~] Aucun joueur à proximitée")
		end
...
Keys.Register('F6', 'unicorn', 'Menu job Unicorn', function()
