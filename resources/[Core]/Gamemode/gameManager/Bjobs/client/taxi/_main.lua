    ...
    while playerData ~= nil and playerData.job ~= nil and playerData.job.name == "taxi" do
        ...
        for i = 1, #storePosition do
            ...
            if (currentPosition ~= nil) then
                ...
                if (currentPedVehicle ~= 0 and DoesEntityExist(currentPedVehicle) and GetEntityModel(currentPedVehicle) == Taxi.Config.vehicle.model and #(playerCoords-currentPosition) < 2.0) then
                    loopInterval = 0
                    ESX.ShowHelpNotification("Appuyez sur ~INPUT_CONTEXT~ pour ranger votre véhicule.")
                    if IsControlJustPressed(0, 51) then
                        TriggerServerEvent("Taxi:Store:Vehicle")
                    end
                elseif (currentPedVehicle ~= 0 and DoesEntityExist(currentPedVehicle) and GetEntityModel(currentPedVehicle) == Taxi.Config.vehicle.model and #(playerCoords-currentPosition) < 10.0) then
                    loopInterval = 0
                    DrawMarker(Config.Get.Marker.Type, currentPosition, 0, 0, 0, Config.Get.Marker.Rotation, nil, nil, Config.Get.Marker.Size[1], Config.Get.Marker.Size[2], Config.Get.Marker.Size[3], Config.Get.Marker.Color[1], Config.Get.Marker.Color[2], Config.Get.Marker.Color[3], 170, 0, 1, 0, 0, nil, nil, 0)
                end
            end
        end
