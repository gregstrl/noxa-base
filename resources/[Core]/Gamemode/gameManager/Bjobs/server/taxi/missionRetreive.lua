...
AddEventHandler("playerDropped", function()
    ...
    -- Vérifie si le joueur a le métier "taxi"
    local playerJob = xPlayer.getJob()
    if not playerJob or playerJob.name ~= "taxi" then return end

    -- Vérifie si le joueur a une mission en cours
    local playerMission = Taxi.Mission.List[xPlayer.identifier]
    if playerMission then
        playerMission:stop(true)
    end
end)