...
AddEventHandler('esx_holdupbank:toofar', function(robb)
	...
	for i = 1, #xPlayers, 1 do
		local xPlayer = ESX.GetPlayerFromId(xPlayers[i])
 		...
 		if xPlayer and xPlayer.job.name == 'lsco' or xPlayer and xPlayer.job.name == 'sahp' or xPlayer and xPlayer.job.name == 'sasp' or xPlayer and xPlayer.job.name == 'usms'  then
...
AddEventHandler('esx_holdupbank:rob', function(robb)
	...
	if Banks[robb] then
		...
		if not rob then
			...
			TriggerClientEvent('esx:showNotification', _source, "~s~Un braquage est déjà en cours.")
		end
	end
end)
...
-- Fonction pour vérifier si le métier est un métier de police
...
local function UpdateCopsCount()
    CopsConnected = 0
...
-- Événements pour maintenir à jour le compteur
...
Citizen.CreateThread(function()
    UpdateCopsCount() -- Initialise le compteur au démarrage du script
end)
	...
	if Stores[robb] then
		...
		if rob == false then
			...
			for i=1, #xPlayers, 1 do
				local xPlayer = ESX.GetPlayerFromId(xPlayers[i])
				if xPlayer and xPlayer.job.name == 'lsco' or xPlayer and xPlayer.job.name == 'sahp' or xPlayer and xPlayer.job.name == 'sasp' or xPlayer and xPlayer.job.name == 'usms'  then
					TriggerClientEvent('esx:showNotification', xPlayers[i], "~s~ Braquage en cours à: ~g~" .. store.nameofstore)
			...
			TriggerClientEvent('esx:showNotification', source, "~s~Un braquage est déjà en cours.")
		end
	end
end)
...
RegisterNetEvent("Koy:ClaquetteChausette", function()
		...
		if (CopsConnected >= ConfigBraco.RequiredCopsSell) then
			if (not playersTimed[xPlayer.identifier] or GetGameTimer() - playersTimed[xPlayer.identifier] > 10000) then
				...
				SetTimeout(10000, function()
					...
					xPlayer.triggerEvent("Koy:ClaquetteChausetteV2");
				end);
				if (item) then
					local reward = math.floor(ConfigBraco.PriceForOneJewel * ConfigBraco.MaxJewelsSell)
					local SellBijoux = vector3(706.669, -966.898, 30.413);
					if #(coords - SellBijoux) < 5 then
						if item.count >= ConfigBraco.MaxJewelsSell then
							...
							FreezeEntityPosition(ped, false);
							xPlayer.showNotification("[~r~Impossible~h~] Vous n'avez pas assez de bijoux sur vous.");
						end
					else
						xPlayer.ban(0, '(Koy:ClaquetteChausette)');
					end
				end
			else
