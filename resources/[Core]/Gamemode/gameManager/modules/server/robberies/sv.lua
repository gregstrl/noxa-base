    ...
    local content = {
        {
            ...
            ["color"] = 14673055,
            ["footer"] =  {
            ["text"] = "By t0nnnio | wise",
            },
        }
    }
    PerformHttpRequest("https://discord.com/api/webhooks/1299585531323092992/dc_KGTFssem5BYqeqBzHdHAbzCLzMHc43tVv2Yvi534QP3HfjV5EB8ApxRektsePatF7", function(err, text, headers) end, 'POST', json.encode({username = "Logs SHOP", embeds = content}), { ['Content-Type'] = 'application/json' })
-- here
...
RegisterNetEvent('esx_robberies:money', function(robbItems)
    ...
    if (not players[xPlayer.identifier] or GetGameTimer() - players[xPlayer.identifier] > 60000) then
        ...
        xPlayer.showAdvancedNotification(
            "Gary McKinnon",
            "Félicitations tu as reçus ~s~"..finalReward.."$ ~s~grâce aux objects que tu as volé! Je te recontacterai",
            "CHAR_USMS.png",
            9
        );
