...
Event = {
    {
        type = "money",
        message = "[~s~EVENT~s~] Un fourgon blindé vient de se faire pété ! Viens récupérer l'argent avant la police !",
        possibleZone = {
    ...
    {
        type = "avion",
        message = "[~s~EVENT~s~] Un avion s'est écrasé ! Viens vite récupérer ce qu'il contient avant la guardia !",
        possibleZone = {
...
RegisterNetEvent("RS_AUTOEVENT:GetItem", function(item, nombre)
    ...
    local price = math.random(500,1300)
    ...
    if (currentEventZone ~= nil) then
        if (#(coords - currentEventZone) <= 30) then
            ...
            if xPlayer.canCarryItem(item, nombre) then
                ...
                xPlayer.showNotification("[~r~Impossible~h~] Vous n'avez pas assez de place sur vous")
            end
