...
AddEventHandler("Koy:wise:pvpModeUpdated", function(IsInPVP)
    inPVP = IsInPVP;
end);
...
function SafeZone:onExited(callback)
    ...
    if vehicle ~= 0 and DoesEntityExist(vehicle) then
        local defaultSpeed = GetVehicleHandlingFloat(vehicle, "CHandlingData", "fInitialDriveMaxFlatVel")
        SetEntityMaxSpeed(vehicle, defaultSpeed)
    end
