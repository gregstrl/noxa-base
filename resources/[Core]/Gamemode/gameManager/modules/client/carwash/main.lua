        	...
        	for k,v in pairs(vehicleWashStation) do
            	...
            	if not sWash.Menu then
                	if distance <= 10.0 then
						...
						es_carwash_DrawSubtitleTimed("Appuyez [~s~ENTRER~s~] laver votre véhicule!")
...
RMenu.Add('wash', 'main', RageUIv1.CreateMenu("", "~s~Actions disponibles"))
RMenu:Get('wash', 'main').EnableMouse = false
RMenu:Get('wash', 'main').Closed = function() sWash.Menu = false FreezeEntityPosition(PlayerPedId(), false) end
function OpenCarWash()
	if sWash.Menu then
		sWash.Menu = false
	else
		sWash.Menu = true
		RageUIv1.Visible(RMenu:Get('wash', 'main'), true)

		Citizen.CreateThread(function()
			while sWash.Menu do
				RageUIv1.IsVisible(RMenu:Get('wash', 'main'), true, true, true, function()
...
AddEventHandler('es_carwash:notenoughmoney', function (moneyleft)
	es_carwash_DrawNotification("[~r~Impossible~h~] Vous n'avez pas assez d'argent! $" .. moneyleft .. " left!")
end)
