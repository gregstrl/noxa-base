...
pSociety.InitRageUIMenu = function(_title, _subtitle, _texturedict, _texturename, _rgb, _banner)
    if _banner then
	    RMenu.Add('bossmenu', 'main', RageUIv1.CreateMenu(_title, _subtitle, nil, nil, _texturedict, _texturename))
    else
        RMenu.Add('bossmenu', 'main', RageUIv1.CreateMenu(_title, _subtitle, 10, 200))
    end
	if _rgb ~= nil then
		RMenu:Get('bossmenu', 'main'):SetRectangleBanner(_rgb[1], _rgb[2], _rgb[3], 500)
	end
    RMenu.Add('bossmenu', 'manage_employees', RageUIv1.CreateSubMenu(RMenu:Get('bossmenu', 'main'), _title, _subtitle))
    RMenu.Add('bossmenu', 'update_employee', RageUIv1.CreateSubMenu(RMenu:Get('bossmenu', 'manage_employees'), _title, _subtitle))
    RMenu.Add('bossmenu', 'manage_salary', RageUIv1.CreateSubMenu(RMenu:Get('bossmenu', 'main'), _title, _subtitle))
                ...
                if not pSociety.Menu then
					if ESX.PlayerData.job ~= nil and ESX.PlayerData.job.name == v.name and ESX.PlayerData.job.grade_name == "boss" then
						if #(pCoords - v.pos) <= 2.5 then
                            ...
                            Draw3DText(v.pos.x, v.pos.y, v.pos.z, "Appuyez sur E pour gérer votre ~s~entreprise")
							DrawMarker(cfg.Type, v.pos.x, v.pos.y, v.pos.z-1, 0.0, 0.0, 0.0, -90.0, 0.0, 0.0, cfg.Scale[1], cfg.Scale[2],cfg.Scale[3], 247,247,247, 170, 0, 0, 0, 1, nil, nil, 0)
...
AddEventHandler("pSociety:SendRequestRecruit", function(bb, cc)
	RageUIv1.PopupChar({
		message = "~s~infos:~s~\n"..bb.."\n\n~s~Y~s~ "..pSociety.Trad["accept"]..". | ~s~G~s~ "..pSociety.Trad["decline"]..".",
