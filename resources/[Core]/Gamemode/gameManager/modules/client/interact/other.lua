...
handsupnow = function()
    if DoesEntityExist(PlayerPedId()) and not IsEntityDead(PlayerPedId()) then
        if not IsPedInAnyVehicle(PlayerPedId(), false) and not IsPedSwimming(PlayerPedId()) and not IsPedShooting(PlayerPedId()) and not IsPedClimbing(PlayerPedId()) and not IsPedCuffed(PlayerPedId()) and not IsPedDiving(PlayerPedId()) and not IsPedFalling(PlayerPedId()) and not IsPedJumpingOutOfVehicle(PlayerPedId()) and not IsPedUsingAnyScenario(PlayerPedId()) and not IsPedInParachuteFreeFall(PlayerPedId()) then
            ...
            if not handsup then
                handsup = true
                TaskPlayAnim(PlayerPedId(), "random@mugging3", "handsup_standing_base", 1.0, -1.0, -1, 49, 0, 0, 0, 0)
                Wait(100) -- Pause pour une transition plus naturelle
                SetPedCurrentWeaponVisible(PlayerPedId(), false, true, true, true) -- Cache l'arme
                TriggerServerEvent("esx_thief:update", true)
            else
                handsup = false
                ClearPedSecondaryTask(PlayerPedId())
                Wait(150) -- Pause pour éviter une coupure brutale
                SetPedCurrentWeaponVisible(PlayerPedId(), true, true, true, true) -- Réaffiche l'arme
...
Keys.Register('Y','handsup', 'Lever les mains', function()
	handsupnow()
end)
...
Keys.Register('W','S\'accroupir', 's\'accroupir', function()
	crouch()
end)
...
RegisterCommand('noir', function()
    ...
    if noir then 
		...
		ExecuteCommand("+notifOff")
		ExecuteCommand("+notifAdvOff")
    end
    while noir do
	...
	ExecuteCommand("+notifOn")
	ExecuteCommand("+notifAdvOn")
