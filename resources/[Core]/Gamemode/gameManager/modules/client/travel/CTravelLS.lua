...
RMenu.Add('razzcayoo', 'menucayoo', RageUIv1.CreateMenu("", "~s~Voici notre destination"))
          ...
          if Distance < 500.0 then
              if Distance < 1 then
                  ...
                  if IsControlJustPressed(1, 38) then
                      RageUIv1.Visible(RMenu:Get('razzcayoo', 'menucayoo'), true)
                      local IsCayooMenuOpen = true
                      while IsCayooMenuOpen do
                          Wait(1)
                          if not RageUIv1.Visible(RMenu:Get('razzcayoo', 'menucayoo')) then
                              IsCayooMenuOpen = false
                          end

                          RageUIv1.IsVisible(RMenu:Get('razzcayoo', 'menucayoo'), true, true, true, function()

                              RageUIv1.ButtonWithStyle("Aller à Los Santos", nil, { RightLabel = "~g~3500$" }, true, function(h, a, s)
                                  if s then
                                      RageUIv1.CloseAll()
                                      ESX.TriggerServerCallback('razzouvoyage:givemoney', function(hasEnoughMoney)
                                          if hasEnoughMoney then
                                              ...
                                              StartPlayerTeleport(PlayerId(), -1039.01, -2739.56, 20.16, 238.437, true, true, false)
                                              Wait(10)
                                              StartCutscene(0)
                                              Wait(10)
                                              ClonePedToTarget(playerClone, plyrId)
