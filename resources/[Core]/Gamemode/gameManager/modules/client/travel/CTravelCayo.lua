...
local lastCayoPosition = nil

local function IsInCayoPericoBounds()
    local coords = GetEntityCoords(PlayerPedId())
    return (coords.x >= IslandBounds.x2 and coords.x <= IslandBounds.x1 and 
            coords.y >= IslandBounds.y2 and coords.y <= IslandBounds.y1)
end

AddEventHandler('onResourceStop', function(resourceName)
    if resourceName == GetCurrentResourceName() and IsInCayoPericoBounds() then
        local coords = GetEntityCoords(PlayerPedId())
        lastCayoPosition = {
            x = coords.x,
            y = coords.y,
...
Citizen.CreateThread(function()
    while true do
        Wait(1000)
        if lastCayoPosition and not IsInCayoPericoBounds() then
            local ped = PlayerPedId()
            SetEntityCoords(ped, lastCayoPosition.x, lastCayoPosition.y, lastCayoPosition.z)
            SetEntityHeading(ped, lastCayoPosition.heading)
            lastCayoPosition = nil
        end
    end
end)
...
RMenu.Add('razzairport', 'menuairport', RageUIv1.CreateMenu("", "~g~Voici notre destination"))
...
CreateThread(function()
    while true do
        ...
        local Distance = Vdist2(GetEntityCoords(PlayerPedId()), -1039.822, -2740.926, 20.1693)
        if Distance < 500.0 then
            if Distance < 1 then
                AddTextEntry("HELP", "Appuyer sur ~INPUT_PICKUP~ pour parler avec la personne.")
                DisplayHelpTextThisFrame("HELP", false)
                if IsControlJustPressed(1, 38) then
                    RageUIv1.Visible(RMenu:Get('razzairport', 'menuairport'), true)
                    local IsLocCayoMenuOpen = true
                    while IsLocCayoMenuOpen do
                        Wait(1)
                        if not RageUIv1.Visible(RMenu:Get('razzairport', 'menuairport')) then
                            IsLocCayoMenuOpen = false
                        end

                        RageUIv1.IsVisible(RMenu:Get('razzairport', 'menuairport'), true, true, true, function()

                            RageUIv1.ButtonWithStyle("Aller à cayo perico", nil, { RightLabel = "~g~3500$" }, true, function(h, a, s)
                                if s then
                                    RageUIv1.CloseAll()
...
zone123 = vector3(-1039.822, -2740.926, 19.1693)
Heading123 = 317.55
Ped123 = nil
