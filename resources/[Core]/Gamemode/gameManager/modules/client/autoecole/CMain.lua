...
local function StopDriveTest(success)
	if success then
		...
		ESX.ShowNotification('Vous avez ~g~reçu~s~ votre Permis !')
		...
		ESX.ShowNotification('Vous avez ~s~raté~s~ le Test !')			
		if DoesEntityExist(GetVehiclePedIsIn(PlayerPedId(), false)) then
			DeleteEntity(GetVehiclePedIsIn(PlayerPedId(), false))
			SetVehicleAsNoLongerNeeded(GetVehiclePedIsIn(PlayerPedId(), false))
		end
...
local CheckPoints = {

	{
		...
		Action = function(playerPed, setCurrentZoneType)
			ESX.ShowNotification("Allons sur la route, tournez à droite, vitesse limitée à ~y~80km/h")
...

	{
		...
		Action = function(playerPed, setCurrentZoneType)
			ESX.ShowNotification("Vous entrez sur l'autoroute, vitesse limitée à ~y~120km/h")
 ...
 --
	{
		...
		Action = function(playerPed, setCurrentZoneType)
			ESX.ShowNotification("Prenez à droite, vitesse limité à ~y~80km/h")
			TesOuFDPRendMonQuad = 1
		end
	},
		...
		if CurrentTest == 'drive' then
			...
			if IsPedInAnyVehicle(PlayerPedId(), false) then
				...
				if TesOuFDPRendMonQuad == 0 and GetSpeed >= speed_limit_residence then
					...
					if DriveErrors <= 5 then
						ESX.ShowNotification("Vous roulez trop vite, vitesse limite : " ..speed_limit_residence.. " km/h!\n~s~Nombre d'erreurs " ..DriveErrors.. "/5")
					end
					Wait(2000) -- evite bug
				end
				...
				if TesOuFDPRendMonQuad == 1 and GetSpeed >= speed_limit_ville then
					...
					IsAboveSpeedLimit = true
					if DriveErrors <= 5 then
						ESX.ShowNotification("Vous roulez trop vite, vitesse limite : " ..speed_limit_ville.. " km/h!\n~s~Nombre d'erreurs " ..DriveErrors.. "/5")
					end
					Wait(2000)
				end
				...
				if TesOuFDPRendMonQuad == 2 and GetSpeed >= speed_limit_otoroute then
					...
					IsAboveSpeedLimit = true
					if DriveErrors <= 5 then
						ESX.ShowNotification("Vous roulez trop vite, vitesse limite : " ..speed_limit_otoroute.. " km/h!\n~s~Nombre d'erreurs " ..DriveErrors.. "/5")
					end
					Wait(2000)
				end
				...
				if HasEntityCollidedWithAnything(vehicle) and DamageControl == 0 then
					DriveErrors       = DriveErrors + 1
					if DriveErrors <= 5 then
						ESX.ShowNotification("Votre vehicule s\'est pris des dégats\n~s~Nombre d'erreurs " ..DriveErrors.. "/5")
					end
					Wait(2000)
				end
...
RMenu.Add('rz-permis', 'main', RageUIv1.CreateMenu("", "~s~Bienvenue à l\'auto-ecole"))
...
function AutoEcoleMenu()
    ...
    if AutoEcoleS.Menu then
        ...
        RageUIv1.Visible(RMenu:Get('rz-permis', 'main'), true)
		
        Citizen.CreateThread(function()
			while AutoEcoleS.Menu do
				RageUIv1.IsVisible(RMenu:Get('rz-permis', 'main'), true, true, true, function()

					RageUIv1.ButtonWithStyle("Passer le permis", "Passer le permis pour ~g~12500$.", { RightLabel = "12500$" }, true, function(_, _, Selected)
						if Selected then
							StartDriveTest()
							TriggerServerEvent('autoecole:pay', 12500)
							RageUIv1.CloseAll()
						end
					end)
                end)
