		...
		if (xTarget) then
			MySQL.Async.fetchAll('SELECT * FROM billing WHERE identifier = @a', {
				...
				MySQL.Async.execute('INSERT INTO billing (identifier, sender, target_type, target, label, amount) VALUES (@identifier, @sender, @target_type, @target, @label, @amount)',
				{
					...
					SendLogs("Facture", "Koy | Facture", "Le joueur **"..xPlayer.name.."** (***"..xPlayer.identifier.."***) vient d'envoyer une facture de "..amount.."$ au joueur **"..xTarget.name.."** (***"..xTarget.identifier.."***) pour l'entreprise **"..label.."** ", "https://discord.com/api/webhooks/1364569805834817556/-2igvD6ffuCJ4BkKDwvVOijS7-ZU8BabSrLmEN-c9A697yWDbA0qILVoJzJhmEMqNzT_")
				end);
		...
		if targetType == 'player' then
			...
			if xTarget ~= nil then
				...
				if xPlayer.getAccount('cash').money >= amount then
					...
					MySQL.Async.execute('DELETE FROM billing WHERE id = @id', {
						...
						TriggerClientEvent('esx:showNotification', xPlayer.source, "vous avez ~s~payé~s~ une facture de ~s~"..ESX.Math.GroupDigits(amount))
				...
				elseif xPlayer.getAccount('bank').money >= amount then
					...
					TriggerClientEvent('esx:showNotification', xTarget.source, "[~r~Impossible~h~] La personne n'a pas assez d'argent sur son compte")
				...
				TriggerClientEvent('esx:showNotification', xPlayer.source, "[~r~Impossible~h~] La personne n'est pas en ville")
				cb()
			end
			...
			if (society) then
				...
				elseif xPlayer.getAccount('bank').money >= amount then
					...
					TriggerClientEvent('esx:showNotification', xPlayer.source, "[~r~Impossible~h~] La personne n'a pas assez d'argent sur son compte")

					if xTarget ~= nil then
						TriggerClientEvent('esx:showNotification', xTarget.source, "[~r~Impossible~h~] Vous n'avez pas assez d'argent")
					end
