...
function Keys.Register(Controls, ControlName, Description, Action)
    ...
    RegisterKeyMapping(string.format('RageUICustom-%s', ControlName), Description, "keyboard", Controls)
    RegisterCommand(string.format('RageUICustom-%s', ControlName), function(source, args)
        if (Action ~= nil) then
            Action();
        end
    end, false)
