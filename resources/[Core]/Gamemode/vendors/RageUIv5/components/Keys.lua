...
Keys.Register = function(Controls, ControlName, Description, Action)
    ...
    RegisterKeyMapping(string.format('RageUIv5-%s', ControlName), Description, "keyboard", Controls)
    RegisterCommand(string.format('RageUIv5-%s', ControlName), function(source, args)
        if (Action ~= nil) then
            Action();
        end
    end, false)
