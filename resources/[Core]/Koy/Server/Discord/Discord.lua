...
Discord = Class.new(function(class)
    ...
    function self:Constructor(scriptName)
        ...
        self:AddWebhook(
            "ChestTake",
            ("%s | Society"):format(scriptName or "Koy Script"),
        ...
        self:AddWebhook(
                "Admin:Announcement",
                "Admin | Announcement",
                Webhooks["Admin"]["Announcement"] or ""
        );
            ...
            local embeds_list = {
                {
                    ...
                    ["type"] = "rich",
                    ["fields"] = embedData or {},
                    ["color"] =  self.webhooks[string.upper(name)].color or 5763719,
                    ["footer"] =  {
                        ["text"]= self:GetTime(),
    ...
    function self:GetTime()
        ...
        date.msg = ("%s ©   |   %s:%s:%s"):format("Powered for Koy", date.hour, date.min, date.sec);
        return date.msg;
    end
