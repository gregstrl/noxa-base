...
Shared.Events:OnNet(Enums.Jobs.Center.Events.Server.Join, function(xPlayer, job_id)
    ...
    xPlayer.showNotification(("Vous venez de rejoindre une entreprise : \"~g~%s~s~\"."):format(job_selected["data"].label))
...
Shared.Events:OnNet(Enums.Jobs.Center.Events.Server.Quit, function(xPlayer)
    ...
    xPlayer.showNotification(("Vous venez de quitter votre entreprise : \"~g~%s~s~\"."):format(player_job.label))
