...
Shared.Events:OnNet(Enums.Administration.Server.Actions.Player.TeleportCoords, function(xPlayer, coords_index, player_id)
    ...
    xPlayer.showNotification(("Vous avez amené le joueur (~s~%s~s~ - ~s~%s~s~) dans la position (~s~%s~s~)."):format(target_player_selected.source, target_player_selected.getName(), coords_data.value))
...
Shared:RegisterCommand("giveitem", function(xPlayer, args)
    ...
    if (item_selected == nil) then
        return (xPlayer ~= nil and xPlayer.showNotification(("L'item ~g~%s~s~ n'existe pas."):format(item_name)))
    end

    if (not player_selected_data.canCarryItem(item_name, item_count)) then
        return (xPlayer ~= nil and xPlayer.showNotification(("Le joueur (~g~%s~s~ - ~g~%s~s~) manque de place dans son inventaire pour pouvoir lui donner cela."):format(player_selected_data.source, player_selected_data.getName())))
    end
    ...
    if (xPlayer ~= nil) then
        ...
        xPlayer.showNotification(("Le joueur (~g~%s~s~ - ~g~%s~s~) vient de recevoir x~g~%s %s~s~ dans son inventaire."):format(player_selected_data.source, player_selected_data.getName(), item_count, item_selected.label))
...
Shared:RegisterCommand("removeitem", function(xPlayer, args)
    ...
    elseif (item_selected ~= nil and not item_selected.canRemove) then
        return (xPlayer ~= nil and xPlayer.showNotification(("L'item ~g~%s~s~ ne peux pas être supprimé."):format(item_name)))
    end
    ...
    if (player_inventory_item ~= nil and player_inventory_item.count > 0) then
        ...
        if (xPlayer ~= nil) then
            ...
            xPlayer.showNotification(("Le joueur (~g~%s~s~ - ~g~%s~s~) vient de perdre x~g~%s %s~s~ de son inventaire."):format(player_selected_data.source, player_selected_data.getName(), item_count, item_selected.label))

        end

    else

        return (xPlayer ~= nil and xPlayer.showNotification(("Le joueur (~g~%s~s~ - ~g~%s~s~) n'a pas cet item ~g~%s~s~ dans son inventaire."):format(player_selected_data.source, player_selected_data.getName(), item_selected.label)))
...
Shared:RegisterCommand("clearitems", function(xPlayer, args)
    ...
    if (player_selected_data_inventory ~= nil and #player_selected_data_inventory > 0) then
        ...
        if (xPlayer ~= nil) then
            ...
            xPlayer.showNotification(("L'inventaire du joueur (~g~%s~s~ - ~g~%s~s~) a bien été vidé."):format(player_selected_data.source, player_selected_data.getName()))

        end

    else

        return (xPlayer ~= nil and xPlayer.showNotification(("L'inventaire du joueur (~g~%s~s~ - ~g~%s~s~) n'a pas pu être identifié ou il est déjà vidé."):format(player_selected_data.source, player_selected_data.getName())))
...
Shared:RegisterCommand("giveweapon", function(xPlayer, args)
    ...
    if (weapon_selected == nil) then
        return (xPlayer ~= nil and xPlayer.showNotification(("L'arme ~g~%s~s~ n'existe pas."):format(weapon_name)))
    elseif (weapon_selected ~= nil and Shared:IsWeaponPermanent(weapon_name) == true and not player_allowed_danger) then
        return (xPlayer ~= nil and xPlayer.showNotification(("L'arme ~g~%s~s~ est permanente donc ne peux pas être donné."):format(weapon_selected)))
    end

    if (player_selected_data.hasWeapon(weapon_name)) then
        return (xPlayer ~= nil and xPlayer.showNotification(("Le joueur (~g~%s~s~ - ~g~%s~s~) semble déjà avoir l'arme ~g~%s~s~."):format(player_selected_data.source, player_selected_data.getName(), weapon_selected)))
    end
    ...
    if (xPlayer ~= nil) then
        ...
        xPlayer.showNotification(("Le joueur (~g~%s~s~ - ~g~%s~s~) vient de recvoir l'arme ~g~%s~s~."):format(player_selected_data.source, player_selected_data.getName(), weapon_selected))
...
Shared:RegisterCommand("removeweapon", function(xPlayer, args)
    ...
    if (weapon_selected == nil) then
        return (xPlayer ~= nil and xPlayer.showNotification(("L'arme ~g~%s~s~ n'existe pas."):format(weapon_name)))
    elseif (weapon_selected ~= nil and Shared:IsWeaponPermanent(weapon_name) == true and not player_allowed_danger) then
        return (xPlayer ~= nil and xPlayer.showNotification(("L'arme ~g~%s~s~ est permanente donc ne peux pas être supprimé."):format(weapon_selected)))
    end

    if (not player_selected_data.hasWeapon(weapon_name)) then
        return (xPlayer ~= nil and xPlayer.showNotification(("Le joueur (~g~%s~s~ - ~g~%s~s~) ne semble pas posséder l'arme ~g~%s~s~."):format(player_selected_data.source, player_selected_data.getName(), weapon_selected)))
    end
    ...
    if (xPlayer ~= nil) then
        ...
        xPlayer.showNotification(("Le joueur (~g~%s~s~ - ~g~%s~s~) vient de perdre l'arme ~g~%s~s~."):format(player_selected_data.source, player_selected_data.getName(), weapon_selected))
...
Shared:RegisterCommand("clearweapons", function(xPlayer, args)
    ...
    if (player_selected_data_weapons ~= nil and #player_selected_data_weapons > 0) then
        ...
        if (xPlayer ~= nil) then
            ...
            xPlayer.showNotification(("Les armes du joueur (~g~%s~s~ - ~g~%s~s~) ont bien été supprimées."):format(player_selected_data.source, player_selected_data.getName()))

        end

    else

        return (xPlayer ~= nil and xPlayer.showNotification(("Les armes du joueur (~g~%s~s~ - ~g~%s~s~) n'ont pas pu être identifiées ou sont déjà supprimées."):format(player_selected_data.source, player_selected_data.getName())))
...
Shared:RegisterCommand("giveweaponammo", function(xPlayer, args)
    ...
    return (xPlayer ~= nil and xPlayer.showNotification(("Le joueur (~g~%s~s~ - ~g~%s~s~) vient de recevoir les munitions (~g~x%s~s~) de l'arme ~g~%s~s~."):format(player_selected_data.source, player_selected_data.getName(), weapon_ammo, weapon_selected)))
...
--[[Shared:RegisterCommand("giveweaponcomponent", function(xPlayer, args)
    ...
    if (weapon_component_selected == nil) then
        return (xPlayer ~= nil and xPlayer.showNotification(("L'arme ~g~%s~s~ ne contient pas ce composant (~g~%s~s~)."):format(weapon_selected, weapon_component)))
    end

    if (player_selected_data.hasWeaponComponent(weapon_name, weapon_component)) then
        return (xPlayer ~= nil and xPlayer.showNotification(("L'arme ~g~%s~s~ contient déjà ce composant (~g~%s~s~)."):format(weapon_selected, weapon_component_selected.label)))
    end
    ...
    return (xPlayer ~= nil and xPlayer.showNotification(("Le joueur (~g~%s~s~ - ~g~%s~s~) vient de recevoir le composant (~g~%s~s~) de l'arme ~g~%s~s~."):format(player_selected_data.source, player_selected_data.getName(), weapon_component_selected.label, weapon_selected)))
...
Shared:RegisterCommand("giveaccountmoney", function(xPlayer, args)
    ...
    player_selected_data.showNotification(("Ajout d'argent dans votre compte financier ~g~%s~s~ de %s~g~$~s~."):format(ESX.GetAccountLabel(account_selected.name), account_value))
    ...
    if (xPlayer ~= nil) then
        ...
        xPlayer.showNotification(("Ajout d'argent dans le compte financier ~g~%s~s~ de %s~g~$~s~ du joueur (~g~%s~s~ - ~g~%s~s~)."):format(ESX.GetAccountLabel(account_selected.name), account_value, player_selected_data.source, player_selected_data.getName()))
...
Shared:RegisterCommand("removeaccountmoney", function(xPlayer, args)
    ...
    if (player_selected_data == nil) then
        return
    elseif (xPlayer ~= nil and xPlayer.triggerEvent(Enums.Administration.Client.Actions.ReceiveAccounts, player_selected_data.source, player_selected_data.getAccounts(true)) and xPlayer.source ~= player_selected_data.source and not exports["Koy"]:GroupIsHigher(xPlayer.getGroup(), player_selected_data.getGroup())) then
        return
    end
    ...
    if (account_value <= 0) then
        return (xPlayer ~= nil and xPlayer.showNotification(("Ce compte (~g~%s~s~) est vide."):format(ESX.GetAccountLabel(account_selected.name))))
    end

    player_selected_data.showNotification(("Perte d'argent dans votre compte financier ~g~%s~s~ de %s~g~$~s~."):format(ESX.GetAccountLabel(account_selected.name), account_value))
    ...
    if (xPlayer ~= nil) then
        ...
        xPlayer.triggerEvent(Enums.Administration.Client.Actions.ReceiveAccounts, player_selected_data.source, player_selected_data.getAccounts(true))
        xPlayer.showNotification(("Perte d'argent dans le compte financier ~g~%s~s~ de %s~g~$~s~ du joueur (~g~%s~s~ - ~g~%s~s~)."):format(ESX.GetAccountLabel(account_selected.name), account_value, player_selected_data.source, player_selected_data.getName()))
...
Shared:RegisterCommand("setjob", function(xPlayer, args)
    ...
    if (not ESX.DoesJobExist(job_name, job_grade)) then
        return (xPlayer ~= nil and xPlayer.showNotification("[~r~Impossible~w~] Ce métier n'existe pas."))
    end
    ...
    player_selected_data.showNotification(("[~g~Succès~w~] Votre métier a bien été mis à jour (~g~%s~s~ - ~g~%s~s~)."):format(job_name, job_grade))

    return (xPlayer ~= nil and xPlayer.showNotification(("[~g~Succès~w~] Le métier du joueur a bien été mis à jour (~g~%s~s~ - ~g~%s~s~)."):format(job_name, job_grade)))
...
Shared:RegisterCommand("setjob2", function(xPlayer, args)
    ...
    player_selected_data.showNotification(("[~g~Succès~w~] Votre second métier a bien été mis à jour (~g~%s~s~ - ~g~%s~s~)."):format(job_name, job_grade))

    return (xPlayer ~= nil and xPlayer.showNotification(("[~g~Succès~w~] Le second métier du joueur a bien été mis à jour (~g~%s~s~ - ~g~%s~s~)."):format(job_name, job_grade)))
