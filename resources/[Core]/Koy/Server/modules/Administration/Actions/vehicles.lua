    ...
    if (name == "vehicle_spawn" or name == "vehicle_give") then
        ...
        elseif (name == "vehicle_take_key") then
            ...
            if (vehicle_key) then
                ...
                xPlayer.showNotification(("Vous avez pris de force la clé du véhicule [~g~%s~s~] de ~y~%s~s~."):format(vehicle_plate, vehicle_author))
...
Shared:RegisterCommand("dv", function(xPlayer, args)
    ...
    if (radius_selected >= 500 and radius_selected <= 0) then
        return
    end
    ...
    if (vehicle_number_deleted > 0) then
        xPlayer.showNotification(("Vous avez supprimé ~g~%s véhicules~s~."):format(vehicle_number_deleted))
    end
