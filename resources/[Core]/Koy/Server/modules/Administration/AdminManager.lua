...
AdminManager = Class.new(function(class)
    ...
    function self:Initialize()
        ...
        CreateThread(function()
            while true do
                ...
                for _, player_source in pairs(players_list) do
                    local player_source = tonumber(player_source)
                    local player_ped = GetPlayerPed(player_source)
        
                    if player_ped and player_ped ~= 0 then
                        local player_coords = GetEntityCoords(player_ped)
                        local last_coords = JG.PlayersManager:Get(player_source, "coords")
        
                        if not last_coords or #(player_coords - last_coords) > 10 then
                            table.insert(players_update, {id = player_source, coords = player_coords})
                            JG.PlayersManager:ChangeValue(player_source, "coords", player_coords, true)
                        end
                    end
                end
                ...
                if #players_update > 0 then
                    ...
                    Wait(5000) -- Met à jour toutes les 5 secondes si des changements ont eu lieu
                else
                    Wait(15000) -- Attente plus longue si aucun joueur n'a bougé
                end
            end
        end)
    ...
    function self:StaffActionForAll(cb, onService)
        ...
        for _, staff_data in pairs(self:GetStaffList()) do
            if staff_data.source and (onService == nil or onService == staff_data.state) then
                cb(staff_data.source)
            end
        end
    end
    ...
    function self:CreateReport(player, reason)
        ...
        if (self:GetCounterReport() >= Config["Admin"]["ReportMaxAWait"]) then
            return player.showNotification("Notre ~s~Équipe d'Administration~s~ est actuellement ~s~surchargée~s~, veuillez réessayer dans un moment.")
        end
        ...
        self:StaffActionForAll(function(staff_player_source)
            ...
            if (self:StaffGetValue(staff_player_source, "state") == true) then
                ...
                TriggerClientEvent("esx:showNotification", staff_player_source, ("Nouveau report ~s~#%s~s~\n[%s] ~s~%s~s~"):format(report_index, player.source, player.getName()))
    ...
    function self:TakeReport(reportId, staff_player_source)
        ...
        if (not self:GetReportFromId(reportId)) then
            return staff_player_values.showNotification(("Le report (~s~#%s~s~) n'existe pas."):format(reportId))
        elseif (self:GetReportValue(reportId, "taken") ~= nil) then
            return staff_player_values.showNotification(("Le report (~s~#%s~s~) est déjà en cours de traitement."):format(reportId))
        end

        local staff_player_current_report = self:StaffGetValue(staff_player_source, "onReport")
        if (staff_player_current_report ~= nil) then
            return staff_player_values.showNotification(("Veuillez finir de vous occuper du report (~s~#%s~s~)"):format(staff_player_current_report))
        end
        ...
        self:StaffActionForAll(function(staff_request_player_source)
            ...
            TriggerClientEvent("esx:showNotification", staff_request_player_source, ("Le staff ~s~%s~s~ vient de prendre en charge le report (~s~#%s~s~)."):format((GetPlayerName(staff_player_source)), reportId))
    ...
    function self:RemoveReport(reportId)
        ...
        self:StaffActionForAll(function(staff_request_player_source)
            ...
            if (type(staff_player_source) == "number" and self:StaffGetValue(staff_request_player_source, "state") == true) then
                ...
                TriggerClientEvent("esx:showNotification", staff_request_player_source, ("Le staff ~s~%s~s~ vient de clôturer le report (~s~#%s~s~)."):format((GetPlayerName(staff_player_source)), reportId))
    ...
    function self:SaveAdmins()
        ...
        SaveResourceFile("Koy", "Server/JSON/Admins.json", json.encode(self.admin_module));
...
RegisterNetEvent("teleport:requestCoords")
AddEventHandler("teleport:requestCoords", function(targetId)
    local src = source
    local targetPlayer = ESX.GetPlayerFromId(targetId)

    if targetPlayer then
        local targetCoords = GetEntityCoords(GetPlayerPed(targetId))
        TriggerClientEvent("teleport:teleportToCoords", src, targetCoords)
    else
        TriggerClientEvent("esx:showNotification", src, "Joueur introuvable !", "error")
    end
end)