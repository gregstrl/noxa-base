...
VehicleManager = Class.new(function(class)
    ...
    function self:CreateVehicle(model, position, heading, plate ,callback, xPlayer)
        local vehicleData = MySQL.query.await('SELECT vehicle FROM owned_vehicles WHERE plate = ?', { plate })
        ...
        vehicle:Spawn(function()
            ...
            if vehicleData and vehicleData[1] then
                vehicle:SetProperties(json.decode(vehicleData[1].vehicle), xPlayer)
            end
