...
Shared.Events:OnNet(Enums.Vehicles.Events.BuyFuel, function(xPlayer, netId, fuelValue, fuelPrice)
    ...
    if (vehicles_in_filling[entity_selected] ~= nil) then
        return xPlayer.showNotification(("Désolé, mais le reservoir de ce véhicule est déjà en cours de remplissage, veuillez patienter (~g~%s~s~)."):format(vehicles_in_filling[entity_selected]:ShowRemaining()))
    end
    ...
    if (buyAccount.list[buyAccount.current] >= fuel_liter_price_calculated) then
        ...
        xPlayer.showNotification(("Le réservoir de votre véhicule est en cours de remplissage, veuillez patienter (~g~%s~s~)."):format(vehicles_in_filling[entity_selected]:ShowRemaining()))
        ...
        return xPlayer.showNotification("Le réservoir de votre véhicule a bien été ~g~rempli~s~ avec succès.")
