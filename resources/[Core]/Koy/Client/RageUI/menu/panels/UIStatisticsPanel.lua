local Statistics = {
    Background = { Dictionary = "commonmenu", Texture = "gradient_bgd", Y = 10, Width = 431, Height = 42 },
    Text = {
...
function Panels:StatisticPanel(Percent, Text, Index)
    ...
    if CurrentMenu ~= nil then
        if CurrentMenu and (Index == nil or (CurrentMenu.Index == Index)) then
            ...
            RenderRectangle(CurrentMenu.X, CurrentMenu.Y + Statistics.Background.Y + CurrentMenu.SubtitleHeight + RageUI.ItemOffset + (RageUI.StatisticPanelCount * 42), Statistics.Background.Width + CurrentMenu.WidthOffset, Statistics.Background.Height, 17, 17, 17, 255)
            RenderText(Text or "", CurrentMenu.X + Statistics.Text.Left.X, (RageUI.StatisticPanelCount * 40) + CurrentMenu.Y + Statistics.Text.Left.Y + CurrentMenu.SubtitleHeight + RageUI.ItemOffset, 0, Statistics.Text.Left.Scale, 245, 245, 245, 255, 0)
            RenderRectangle(CurrentMenu.X + RageUI.Settings.Items.Title.Background.Width - BarWidth - Statistics.Bar.Right + CurrentMenu.WidthOffset, (RageUI.StatisticPanelCount * 40) + CurrentMenu.Y + Statistics.Bar.Y + CurrentMenu.SubtitleHeight + RageUI.ItemOffset, Percent * BarWidth, Statistics.Bar.Height, 0, 85, 255, 255)
            for i = 1, #Statistics.Divider, 1 do
                RenderRectangle((CurrentMenu.X + RageUI.Settings.Items.Title.Background.Width - BarWidth - Statistics.Bar.Right) + i * ((BarWidth - (#Statistics.Divider / Statistics.Divider[i].Width)) / (#Statistics.Divider + 1)) + CurrentMenu.WidthOffset, (RageUI.StatisticPanelCount * 40) + CurrentMenu.Y + Statistics.Divider[i].Y + CurrentMenu.SubtitleHeight + RageUI.ItemOffset, Statistics.Divider[i].Width, Statistics.Divider[i].Height, 17, 17, 17, 255)
            end
...
function Panels:StatisticPanelAdvanced(Text, Percent, RGBA1, Percent2, RGBA2, RGBA3, Index)
    ...
    if CurrentMenu ~= nil then
        if CurrentMenu and (Index == nil or (CurrentMenu.Index == Index)) then
            ...
            RGBA2 = RGBA2 or { 0, 85, 255, 255 }
            RGBA3 = RGBA3 or { 0, 85, 255, 255 }
