...
Blip = Class.new(function(class) 
    ...
    function self:SetLabel()
        ...
        if self.type == "job" then 
            ...
            AddTextEntry('BLIP'..self.name, '~s~Entreprise~c~:~s~ ~a~')
            --newLabel = ("~c~[~s~ENTREPRISE~c~]~s~ %s"):format(self.label)
        elseif self.type == "farm" then 
            --SetBlipCategory(self.id, 7)
            AddTextEntry('BLIP'..self.name, '~g~Farming~c~:~s~ ~a~')
            --newLabel = ("~c~[~g~FARM~c~]~s~ %s"):format(self.label)
        elseif self.type == "service" then 
            AddTextEntry('BLIP'..self.name, '~g~Service~c~:~s~ ~a~')
            --SetBlipCategory(self.id, 11)
            --newLabel = ("~c~[~g~SERVICE~c~]~s~ %s"):format(self.label)
        else
