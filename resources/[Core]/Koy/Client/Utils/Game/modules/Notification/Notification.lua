...
Notification = Class.new(function(class) 
    ...
    function self:Advanced(sender, subject, message, textureDict, iconType, flash, saveToBrief, hudColorIndex)
        ...
        AddTextEntry('KoyAdvancedNotification', message);
        BeginTextCommandThefeedPost('KoyAdvancedNotification');
    ...
    function self:ShowHelp(message, thisFrame, beep, duration)
        ...
        AddTextEntry('KoyHelpNotification', message);

        if (thisFrame) then

            DisplayHelpTextThisFrame('KoyHelpNotification', false);

        else

            BeginTextCommandDisplayHelp('KoyHelpNotification');
