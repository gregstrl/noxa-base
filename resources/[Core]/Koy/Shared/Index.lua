...
---Koy resource main table (Client, Server, Shared) Made for Koy
JG = {};
...
Shared = Class.new(function(class)
            ...
            if (IsDuplicityVersion()) then
                ...
                if (Player ~= nil and (type(adminOnly) == "table" or type(adminOnly) == "boolean")) then
                    ...
                    if (not JG.AdminManager:PlayerIsStaff(Player)) then
                        ...
                        if (type(adminOnly) ~= "boolean" and adminOnly.group ~= nil and Player.getGroup() ~= adminOnly.group and not JG.AdminManager:GroupIsHigher(Player.getGroup(), adminOnly.group, adminOnly.group_equal)) then
    ...
    function self:ServerColor()
        return self.colors[Config["ServerColor"]] or "~b~";
    end
    ...
    function self:Require(filePath)
        ...
        if (filePath) then
            ...
            elseif (type(filePath) == "string") then
                ...
                local file = LoadResourceFile("Koy", filePath);
