    ...
    if CurrentMenu ~= nil then
        if CurrentMenu() then
            if CurrentMenu.Open then
                ...
                if Controls.PageUp.Enabled then
					for Index = 1, #Controls.PageUp.Keys do
						if not Controls.PageUp.Pressed then
							if IsDisabledControlJustPressed(Controls.PageUp.Keys[Index][1], Controls.PageUp.Keys[Index][2]) then
								Controls.PageUp.Pressed = true
								CurrentMenu.Index = 1
								Citizen.CreateThread(function()
									Citizen.Wait(175)
									Controls.PageUp.Pressed = false
								end)
								break
							end
						end
				...
				if Controls.PageDown.Enabled then
					for Index = 1, #Controls.PageDown.Keys do
						if not Controls.PageDown.Pressed then
							if IsDisabledControlJustPressed(Controls.PageDown.Keys[Index][1], Controls.PageDown.Keys[Index][2]) then
								Controls.PageDown.Pressed = true
								CurrentMenu.Index = CurrentMenu.Options
								Citizen.CreateThread(function()
									Citizen.Wait(175)
									Controls.PageDown.Pressed = false
								end)
								break
							end
						end
