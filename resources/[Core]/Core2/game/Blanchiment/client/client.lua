...
local izBlanchiment = RageUI.CreateMenu("", "Blanchiment", 0, 0, 'k2r', 'interaction_bgd')
...
AddEventHandler("Koy:Koy:pvpModeUpdated", function(inPVP)
    IsInPVP = inPVP;
end);
				...
				RageUI.IsVisible(izBlanchiment,function()
					if ConfigBlackListeJob[ESX.PlayerData.job and ESX.PlayerData.job.name] then
						...
						if passblanchiment == true then  
							...
							RageUI.Button("Tu n'a pas le Minimum", nil, {RightLabel = "~s~"..blackmoney.. " $"}, passblanchiment == true , {
								onSelected = function()
									RageUI.CloseAll()
								end
							})
		...
		for _,v in pairs (ConfigWashMoney.ConfigBlanchiment) do 
			...
			if dif < 10 then 
				...
				if dif < 1.5 and not RageUI.Visible(izBlanchiment) then
					...
					if ESX.PlayerData.job2 and ESX.PlayerData.job2.name == ConfigWashMoney.job2 then
