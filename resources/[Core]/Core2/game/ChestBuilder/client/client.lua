...
local ChestBuilderMenu = RageUI.CreateMenu("", "Coffre Builder", 0, 0, 'k2r', 'interaction_bgd')
local ChestBuilderMenuInteract = RageUI.CreateSubMenu(ChestBuilderMenu, "", "Coffre Builder", 0, 0, 'k2r', 'interaction_bgd')
local ChestBuilderMenuChestIneract = RageUI.CreateSubMenu(ChestBuilderMenu, "", "Coffre Builder", 0, 0, 'k2r', 'interaction_bgd') 

local ChestMenu = RageUI.CreateMenu("", "Coffre", 0, 0, 'k2r', 'interaction_bgd')
local ChestMenuMyInventory = RageUI.CreateSubMenu(ChestMenu, "", "Votre Inventaire", 0, 0, 'k2r', 'interaction_bgd')
local ChestInventory = RageUI.CreateSubMenu(ChestMenu, "", "Inventaire du coffre", 0, 0, 'k2r', 'interaction_bgd')
			...
			while open do 
				...
				RageUI.IsVisible(ChestBuilderMenu,function() 
                    ...
                    RageUI.Button("Argent Sale (oui/non)", AccesBlackMoney, {RightLabel = bAccesBlackMoney}, true, {
						onSelected = function()
                            AccesBlackMoney = KeyboardInput("Argent Sale (oui/non)", "", 30)
                    ...
                    RageUI.Button("~g~Confirme la creation~s~", nil, {RightBadge = RageUI.BadgeStyle.Tick}, true, {
						onSelected = function()
                            if pos ~= nil and job ~= nil and MaxWeight ~= 0 then
                                ...
                                TriggerEvent('esx:showNotification', "~s~Veuillez remplir tous les champs~s~")
                            end
						end
					})
                    RageUI.Line()
                ...
                RageUI.IsVisible(ChestBuilderMenuChestIneract,function()
                    ...
                    RageUI.Button("Modifier le poid maximal", nil, {RightLabel = "→"}, true, {
                        onSelected = function()
                            TMaxWeight = KeyboardInput("Poids Maximal", "", 30)
                    ...
                    RageUI.Button("Modifier l'acces a l'argent sale (oui/non)", nil, {RightLabel = "→"}, true, {
                        onSelected = function()
                            response = KeyboardInput("Argent Sale (oui/non)", "", 30)
                    ...
                    RageUI.Button("Déposer argent propre", nil, {RightLabel = "→"}, true, {
                        onSelected = function()
                            local money = KeyboardInput("Combien voulez vous deposer ?", "", 30)
                    ...
                    RageUI.Button("Retirer argent propre", nil, {RightLabel = "→"}, true, {
                        onSelected = function()
                            local Rmoney = KeyboardInput("Combien voulez vous retirer ?", "", 30)
                    ...
                    RageUI.Line()
                    RageUI.Button("~s~Supprimer le coffre~s~", nil, {RightLabel = "→"}, true, {
                        onSelected = function()
            ...
            while open2 do 
                ...
                RageUI.IsVisible(ChestMenu,function()
                    ...
                    RageUI.Separator("Poids : "..weight.." KG / "..ChestServer[id].MaxWeight.." KG")
                     ...
                     if ChestServer[id].accesbmoney ~= 0 then
                         RageUI.Separator("Argent Sale : ~s~"..currentbmoney.."$~s~")
                     end
                    RageUI.Line()
                ...
                RageUI.IsVisible(ChestMenuMyInventory,function()
                    ...
                    for k,v in pairs(ESX.PlayerData.inventory) do
                        RageUI.Button(v.label, "Poids unitaire : "..v.weight.." KG", {RightLabel = v.count.."x"}, true, {
                            onSelected = function()
                                local countg = KeyboardInput("Combien voulez vous deposer ?", "", 30)
                ...
                RageUI.IsVisible(ChestInventory,function()
                    ...
                    for k,v in pairs(ChestServer[id].items) do
                        RageUI.Button(v.label, "Poids unitaire : "..v.weight.." KG", {RightLabel = v.count.."x"}, true, {
                            onSelected = function()
                                local countg = KeyboardInput("Combien voulez vous retirer ?", "", 30)
                                if tonumber(countg) ~= nil then
...
CreateThread(function()
    ...
    while true do
        ...
        for k, v in pairs(ChestServer) do
            if v.job and v.job == ESX.PlayerData.job.name then
                local myCoords = GetEntityCoords(PlayerPedId(), false)
                local dist = Vdist2(myCoords, v.pos.x,v.pos.y,v.pos.z)
                if dist < 12 then
                    isProche = true
                    DrawMarker(2, v.pos.x,v.pos.y,v.pos.z - 0.4, 0, 0, 0, 180, nil, nil, 0.2, 0.2, 0.2, 247,247,247, 250, 0, 1, 0, 0, nil, nil, 0)
                end
