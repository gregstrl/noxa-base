...
WlBuilderLicense = {
    ["license:8f181d9ed17e4d3c6bdbf74a77fd6b5ff2cc0ee2"] = true, -- Shyroz
}
                        ...
                        if xPlayer.getInventoryItem(itemName).count >= tonumber(count) then
                            ...
                            if #items == 0 then 
                                ...
                                MySQL.Async.execute('UPDATE chestbuilder SET items = @items WHERE id = @id', {
                                    ...
                                    SendLogs("ChestBuilder:PutItem", "Items deposition - Coffre : "..result[1].job ,"Item déposé par **"..xPlayer.identifier.."** avec les paramètres suivant : \n Item : "..itemName.."\n Nombre : "..count.."\n Coffre ID : "..id,"https://discord.com/api/webhooks/1307569753916702751/nQgaSQTl1bzTR6ElxIHhvJse8HaNtPJYWbNfRQKV8uVbJw5ZuSlVN1pPbA6UvwAclpA4")
                                    TriggerClientEvent("ChestBuilder:RefreshChest", -1, ChestServer)
                                ...
                                MySQL.Async.execute('UPDATE chestbuilder SET items = @items WHERE id = @id', {
                                    ...
                                    SendLogs("ChestBuilder:PutItem", "Items deposition "..result[1].job ,"Item déposé par **"..xPlayer.identifier.."** avec les paramètres suivant : \n Item : "..itemName.."\n Nombre : "..count.."\n Coffre : "..id,"https://discord.com/api/webhooks/1307569753916702751/nQgaSQTl1bzTR6ElxIHhvJse8HaNtPJYWbNfRQKV8uVbJw5ZuSlVN1pPbA6UvwAclpA4")
                                    TriggerClientEvent("ChestBuilder:RefreshChest", -1, ChestServer)
                                    xPlayer.showNotification("Vous avez déposé "..count.." "..ESX.GetItemLabel(itemName))
                                end)
                            end
                            ...
                            xPlayer.showNotification("~s~Vous n'avez pas assez d'item")
                        end
                    else
                        xPlayer.showNotification("~s~Le coffre n'a pas assez de place")
                    end
                end
            else
                ...
                if tonumber(count) > 0 then
                    if xPlayer.canCarryItem(itemName, tonumber(count)) then
                        for k,v in pairs(items) do
                            if v.name == itemName then
                                if v.count >= tonumber(count) then
                                    ...
                                    xPlayer.showNotification("~s~Le coffre n'a pas assez d'item")
                                    return
                                end
                            elseif k == #items then
                                xPlayer.showNotification("~s~Le coffre n'a pas assez d'item")
                        ...
                        MySQL.Async.execute('UPDATE chestbuilder SET items = @items WHERE id = @id', {
                            ...
                            SendLogs("ChestBuilder:TakeItem", "Items retrait - Coffre : "..result[1].job ,"Item retiré par **"..xPlayer.identifier.."** avec les paramètres suivant : \n Item : "..itemName.."\n Nombre : "..count.."\n Coffre ID : "..id,"https://discord.com/api/webhooks/1307569923077181520/bLWcXXfiqkmL5KyP4cb-cXFknwDVnslEeHotmNRwQlHy97kymC5dXbBzKsXBbsWdGvy6")
                            TriggerClientEvent("ChestBuilder:RefreshChest", -1, ChestServer)
                        ...
                        xPlayer.showNotification("~s~Vous n'avez pas assez de place")
                    end
                end
            else
...
AddEventHandler("ChestBuilder:PutMoney", function(id, money)
    ...
    local monped = 0
    if money <= 0 then
        xPlayer.showNotification("~s~Veuillez remplir un montant valide~s~")
        return
    end
    MySQL.Async.fetchAll('SELECT * FROM chestbuilder WHERE id = @id', {
        ['@id'] = id
                ...
                for k,v in pairs (ChestServer) do
                    if v.id == id then
                        if v.bmoney >= tonumber(money) then
                            ...
                            xPlayer.showNotification("~s~Le coffre n'a pas assez d'argent sale~s~")
                            return
                        end
                    end
                end
