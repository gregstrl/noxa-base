function RageUI.CreateMenu(Title, Subtitle, X, Y, TextureDictionary, TextureName, R, G, B, A)
    ...
    Menu.Index = 1
    Menu.Sprite = { Dictionary = "k2r", Texture = TextureName or "interaction_bgd", Color = { R = R, G = G, B = B, A = A } }
    Menu.Rectangle = nil
...
function RageUI.Menus:EditSpriteColor(R, G, B, A)
    if self.Sprite.Dictionary == "k2r" then
...
function RageUI.Menus:SetSpriteBanner(TextureDictionary, Texture)
    self.Sprite = { Dictionary = TextureDictionary or "k2r", Texture = Texture or "interaction_bgd" }
    self.Rectangle = nil
end
