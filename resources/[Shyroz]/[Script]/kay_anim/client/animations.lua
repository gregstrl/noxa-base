...
local USING_INVENTORY <const> = GetResourceState("inventory"):find("start") and GetConvarInt("inventory:weaponanims", 1) ~= 1
local complete = true
...
local function startWaponAnim(ped, info, weapon, hideWeapon)
    ...
    SetTimeout(info.clothing or info.duration/2, function()
        TriggerEvent("kay_animupdateHolster", weapon, hideWeapon)
    end)
...
if USING_INVENTORY then
    local items = exports.inventory:Items()

    AddEventHandler("inventory:usedItem", function(name, slotId, metadata)
    ...
    AddEventHandler("inventory:currentWeapon", function(weapon)
