...
local function ManageFuelUsage(vehicle)
	...
	if IsVehicleEngineOn(vehicle) then
		local usage = Config.FuelUsage[Round(GetVehicleCurrentRpm(vehicle), 1)] or 0.0
		local classFactor = (Config.Classes[GetVehicleClass(vehicle)] or 1.0)
		local consumptionMultiplier = Config.ConsumptionMultiplier or 1.0
		SetFuel(vehicle, GetVehicleFuelLevel(vehicle) - (usage * classFactor * consumptionMultiplier) / 10)
	end
end
	...
	while true do
		...
		if pumpDistance < 2.5 then
			...
			if Config.UseESX then
				...
				-- Calculer le total des comptes cash et dirtycash
				for i=1, #playerData.accounts, 1 do
					if playerData.accounts[i].name == 'cash' or playerData.accounts[i].name == 'dirtycash' then
						currentCash = currentCash + playerData.accounts[i].money
					end
				end
			end
		else
...
CreateThread(function()
    if Config.ShowAllGasStations and Config.GasStations then
        for _, gasStationCoords in pairs(Config.GasStations) do
            ...
            AddTextComponentString("Xero - Station")
