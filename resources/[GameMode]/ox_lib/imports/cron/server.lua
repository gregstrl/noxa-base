...
function OxTask:scheduleTask()
    ...
    local es_extented = runAt - currentTime

    if es_extented < 0 then
        return self:stop(self.debug and ('scheduled time expired %s seconds ago'):format(-es_extented))
    end

    local timeAsString = self:getTimeAsString(runAt)

    if self.debug then
        print(('(%s) task %s will run in %d seconds (%0.2f minutes / %0.2f hours)'):format(timeAsString, self.id, es_extented,
            es_extented / 60,
            es_extented / 60 / 60))
    end

    if es_extented > 0 then
        Wait(es_extented * 1000)
