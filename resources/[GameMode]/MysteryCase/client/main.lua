...
RegisterCommand(AK4Y.OpenCommand, function()
    if openMenuSpamProtect < GetGameTimer() then 
        ...
        ESX.TriggerServerCallback("KoyCase:getPlayerDetails", function(result)
            local firstname = ""
            local lastname = ""
...
RegisterNUICallback("collectItem", function(data, cb)
    if collectItemSpamProtect < GetGameTimer() then 
        ...
        if lastSelected.itemName == data.collectedItem.itemName then 
            ESX.TriggerServerCallback("KoyCase:collectItem", function(result)
                SendNUIMessage({
                    type = 'setLastItems', 
...
RegisterNUICallback("sellItem", function(data, cb)
    if sellItemSpamProtect < GetGameTimer() then 
        ...
        if lastSelected.sellCredit == data.sellItem.sellCredit then 
            ESX.TriggerServerCallback("KoyCase:sellItem", function(result)
                refreshCoins()
                cb(result)
    ...
    if openCaseSpamProtect < GetGameTimer() then 
        ...
        if data then 
            ...
            if lastValidatedCase then 
                ...
                ESX.TriggerServerCallback("KoyCase:selectedCaseOpen", function(result)
                    if result then
                        refreshCoins()
                        cb(lastSelected)
                    else
...
RegisterNUICallback('sendInput', function(data, cb)
	if sendInputSpamProtect <= GetGameTimer() then
		...
		ESX.TriggerServerCallback("KoyCase:sendInput", function(result)
			if result then 	
				cb(tonumber(result))
			else
				cb(false)
...
RegisterNetEvent('KoyCase:serverNotif')
AddEventHandler('KoyCase:serverNotif', function(data)
	SendNUIMessage({
        type = 'serverNotif', 
        notifInfo = data,
    })	
...
--                 TriggerServerEvent('KoyCase:addGoldCoin', AK4Y.PlayTimeRewardCoin)
--             else
--                 TriggerServerEvent('KoyCase:addSilverCoin', AK4Y.PlayTimeRewardCoin) 
--             end
--         end
--     end
-- end)
