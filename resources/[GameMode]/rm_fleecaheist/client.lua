        ...
        if status then
            ESX.TriggerServerCallback('fleecaheist:server:checkTime', function(time)
                if time then
                    ...
                    local street = GetStreetNameFromHashKey(GetStreetNameAtCoord(pedCo.x, pedCo.y, pedCo.z))
                    local zone = GetLabelText(GetNameOfZone(pedCo.x, pedCo.y, pedCo.z))
                    local report = {
                        type = "Braquage Fleeca",
                        coords = pedCo,
                        street = street,
                        zone = zone,
                        description = "Braquage en cours à la Fleeca"
                    }
                    TriggerServerEvent('sasp:reportShooting', pedCo, street, zone, report)
                    TriggerServerEvent('lsco:reportShooting', pedCo, street, zone, report)
...
function CashAppear(grabModel)
    ...
    if grabModel == 'hei_prop_heist_cash_pile' then
