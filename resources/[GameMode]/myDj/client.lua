...
function volumeUp(DJName)
    if xSound:soundExists(DJName) then
        if currentDJ.volume + 0.1 >= 1 then
            currentDJ.volume = 0.5
            xSound:setVolume(DJName, 0.5)
            --ShowNotification('~s~Maximum volume reached')
        else
            currentDJ.volume = currentDJ.volume + 0.1
            xSound:setVolume(DJName, currentDJ.volume + 0.1)
            --ShowNotification('~s~Volume set to: ' .. (currentDJ.volume + 0.1) * 100 .. '%')
        end
...
function volumeDown(DJName)
    if xSound:soundExists(DJName) then
        ...
        if currentDJ.volume - 0.02 <= 0 then
            currentDJ.volume = 0.0
            xSound:setVolume(DJName, 0.0)
           -- ShowNotification('~s~Minimum volume reached')
        else
            currentDJ.volume = currentDJ.volume - 0.02
            xSound:setVolume(DJName, currentDJ.volume - 0.02)
           -- ShowNotification('~s~Volume set to: ' .. (currentDJ.volume - 0.02) * 100 .. '%')
        end
