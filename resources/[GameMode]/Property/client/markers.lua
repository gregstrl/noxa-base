				...
				for t,b in pairs (UsersInfos) do
					if v.propertyEntering ~= nil then 
						if #(pCoords - vector3(v.propertyEntering.x, v.propertyEntering.y, v.propertyEntering.z)) < 15.0 then
							OnMarker = true
							DrawMarker(2, vector3(v.propertyEntering.x, v.propertyEntering.y, v.propertyEntering.z), 0, 0, 0, 0, 0, 0, 0.25, 0.25, 0.25, 247,247,247, 200, 1, 0, 0, 2)
						end
					...
					if v.propertyGarage ~= nil then 
						if v.propertyOwner == "-" or v.propertyOwner == b.identifier then 
							if #(pCoords - vector3(v.propertyGarage.x, v.propertyGarage.y, v.propertyGarage.z)) < 15.0 then
								OnMarker = true
								DrawMarker(2, vector3(v.propertyGarage.x, v.propertyGarage.y, v.propertyGarage.z), 0, 0, 0, 0, 0, 0, 0.25, 0.25, 0.25, 247,247,247,200, 1, 0, 0, 2)
							end
					...
					if v.propertyOwner == b.identifier then  
						if v.propertyRented ~= nil then 
							if #(pCoords - vector3(v.propertyRented.x, v.propertyRented.y, v.propertyRented.z)) < 15.0 then
								OnMarker = true
								DrawMarker(2, vector3(v.propertyRented.x, v.propertyRented.y, v.propertyRented.z), 0, 0, 0, 0, 0, 0, 0.25, 0.25, 0.25, 247,247,247,200, 1, 0, 0, 2)
							end
							if #(pCoords - vector3(v.propertyRented.x, v.propertyRented.y, v.propertyRented.z)) < 1.2 then
								ESX.ShowHelpNotification("Appuyez sur ~INPUT_PICKUP~ pour ranger votre véhicule dans ~r~"..v.propertyLabel)
								if IsControlJustPressed(1, 51) then
				...
				for k,v in pairs(myProperties) do 
					for t,b in pairs(Config.GarageInteriors) do 
						if #(pCoords - b.Exit) < 15.0 then
							OnMarker = true
							DrawMarker(2, b.Exit, 0, 0, 0, 0, 0, 0, 0.25, 0.25, 0.25, 247,247,247,200, 1, 0, 0, 2)
						end
						...
						if not InVisit then 
							if #(pCoords - b.Computer) < 15.0 then
								OnMarker = true
								DrawMarker(2, b.Computer, 0, 0, 0, 0, 0, 0, 0.25, 0.25, 0.25, 247,247,247,200, 1, 0, 0, 2)
							end
							if #(pCoords - b.Computer) < 1.2 then
								ESX.ShowHelpNotification("Appuyer sur ~INPUT_PICKUP~ pour accéder à la ~r~gestion~s~ de votre garage.")
								if IsControlJustPressed(1, 51) then
				...
				for k,v in pairs (myVehicles) do 
					if IsPedSittingInAnyVehicle(PlayerPedId()) then
						if v.plate == GetVehicleNumberPlateText(GetVehiclePedIsIn(PlayerPedId(), false)) then 
							...
							ESX.ShowHelpNotification(("Appuyez sur ~INPUT_PICKUP~ pour sortir avec ~r~%s~s~."):format(GetVehicleLabel(PropertiesOfVehicle.model)))
							if IsControlJustPressed(1, 51) then 
					...
					for _, interior in pairs(Config.PropertiesInteriors) do 
						if v.propertyInteriors == interior.IdType then 
							if #(pCoords - interior.Exit) < 15.0 then
								OnMarker = true
								DrawMarker(2, interior.Exit, 0, 0, 0, 0, 0, 0, 0.25, 0.25, 0.25, 247,247,247,200, 1, 0, 0, 2)
							end
						end
						...
						if not InVisit then 
							if #(pCoords - interior.Storage) < 15.0 then
								OnMarker = true
								DrawMarker(2, interior.Storage, 0, 0, 0, 0, 0, 0, 0.25, 0.25, 0.25, 247,247,247,200, 1, 0, 0, 2)
							end
							if #(pCoords - interior.Storage) < 1.2 then
								ESX.ShowHelpNotification("Appuyer sur ~INPUT_PICKUP~ pour accéder au coffre.")
								if IsControlJustPressed(1, 51) then
									TriggerServerEvent("Property:CheckStorageAccess", v.propertyID)
								end                                
							end 
						end
					end
