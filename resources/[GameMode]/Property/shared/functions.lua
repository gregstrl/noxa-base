...
LookInfos = function(args)
	if type(args) == "number" then 
		if not args or not type(args) == "number" then return "Aucun" else return ""..math.round(args).."$~s~" end
	else
		if not args or args == "" then return "Non-Défini" else return ""..args.."~s~" end
	end
end
...
getData = function(propertyID)
    ...
    for k, v in pairs(myProperties) do
        if v.propertyID == propertyID then
            if v.data ~= nil and type(v.data) == "table" then
                DataStorage = v.data
            else
                DataStorage = {} -- ⚠️ Évite que DataStorage soit nil
            end
        end
    end
end
...
Announce = function(Index)
    if Index ~= 3 then
        ...
        local Reason = exports.input:ShowSync("Indiquer votre annonce", false, 150, "small_text")
...
countData = function(propertyID)
    ...
    for k, v in pairs(myProperties) do
        if v.propertyID == propertyID then
            countStorage.max = v.maxStorage or 250  
            if v.data ~= nil and type(v.data) == "table" then
                for t, b in pairs(v.data) do
                    if v.data[t].count then
                        counterData = counterData + tonumber(v.data[t].count) or 0
                    end
                end
            end

            if v.data["weapons"] ~= nil and type(v.data["weapons"]) == "table" then
                counterData = counterData + #v.data["weapons"]
            end
        end
    end

    countStorage.count = tonumber(counterData) or 0
    WaitStorage = true
end
...
AddEventHandler("Property:DringDringReponse", function(Sonnerie)
	...
	for k,v in pairs (GetInvite) do 
		ESX.ShowNotification(""..v.playerName.." sonne à votre porte rendez-vous vers votre porte pour lui ouvrir.")
