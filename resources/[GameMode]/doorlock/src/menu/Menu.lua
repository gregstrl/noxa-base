...
function RageUI.CreateMenu(Title, Subtitle, X, Y, TextureDictionary, TextureName, R, G, B, A)
    ...
    Menu.Options = 0
    Menu.Closable = true
    Menu.InstructionalScaleform = RequestScaleformMovie("nil")
    Menu.CursorStyle = 1

    if not TextureDictionary then 
        Menu.Rectangle = { R = 0, G = 0, B = 0, A = 255 }
        Menu.Sprite = nil
    else
        Menu.Rectangle = nil
        Menu.Sprite = { Dictionary = TextureDictionary or "RageUI", Texture = TextureName or "interaction_bgd", Color = { R = R, G = G, B = B, A = A } }
    end
...
function RageUI.Menus:EditSpriteColor(color)
    if self.Sprite.Dictionary == "ogek" then
...
function RageUI.Menus:SetSpriteBanner(TextureDictionary, Texture)
    self.Sprite = { Dictionary = TextureDictionary or "RageUI", Texture = Texture or "interaction_bgd" }
    self.Rectangle = nil
end
...
function RageUI.Menus:UpdateInstructionalButtons(Visible)
    ...
    BeginScaleformMovieMethod(self.InstructionalScaleform, "nil")
    ScaleformMovieMethodAddParamInt(-1)
    EndScaleformMovieMethod()
end
