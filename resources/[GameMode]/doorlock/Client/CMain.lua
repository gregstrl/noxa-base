...
local function checkJobAccess(jobs)
...
function OpenDoorlock()
    ...
    local jobs_submenu = RageUI.CreateSubMenu(door_main, "", "Jobs autorisés")
        ...
        RageUI.Visible(door_main, not RageUI.Visible(door_main))
            ...
            RageUI.IsVisible(door_main, true, true, true, function()
                ...
                RageUI.ButtonWithStyle("Jobs autorisés", nil, {RightLabel = "→→→"}, true, function(Hovered, Active, Selected)
                    if Selected then
                        RageUI.Visible(jobs_submenu, true)
                    end
                end, jobs_submenu)
            ...
            RageUI.IsVisible(jobs_submenu, true, true, true, function()
                RageUI.ButtonWithStyle("Ajouter un job", nil, {RightLabel = "→→→"}, true, function(Hovered, Active, Selected)
                    if Selected then 
                        Job = KeyboardInput("Nom du job", "", 15)
                        if Job == nil then
                            ESX['ShowNotification']('Job invalide')
                        else
                ...
                for k, v in pairs(allowedJobs) do
                    RageUI.ButtonWithStyle("Job: " .. v, nil, {RightLabel = "❌"}, true, function(Hovered, Active, Selected)
                        if Selected then
            ...
            if not RageUI.Visible(door_main) and not RageUI.Visible(jobs_submenu) then
...
RegisterNetEvent("doorlock:client:deleteDoor", function()
    _CreateThread(function()
        while true do
            ...
            RageUI.Text({message = "~s~Appuyez sur [~b~E~s~] pour retirer la porte"})
            ...
            while true do
                ...
                if IsEntityAnObject(entity) then
                    ...
                    if #_doorsDobule ~= 2 then
                        ...
                        DrawLine(_coords, coords, 0, 255, 34, 255)
                        RageUI.Text({message = "~s~Appuyez sur ~g~[~s~E~g~]~s~ ~s~pour confirmer"})
                    end
                    showedEntity = entity
                    if IsControlJustPressed(1, 51) then
                        local _doorCoords = GetEntityCoords(entity)
...
RegisterCommand("lockdoor", function()
    if show then
        local _allowed = checkJobAccess(_selectedDoorJobs)
        if _allowed then
            pulsed = true
            return
        end
