...
local function TestCar(pPos)
    ...
    while test do
        ...
        while IsPedInAnyVehicle(PlayerPedId()) == false do
            ...
            ESX.ShowNotification("~b~Vous êtes descendu du véhicule.")
            test = false
        ...
        if result == 0 then
            ...
            ESX.ShowNotification("Votre test est terminé.")
            test = false
            SetEntityCoords(PlayerPedId(), pPos.x, pPos.y, pPos.z)
            break
        end
...
local select = {}
local open  = false
local mainMenu = RageUI.CreateMenu("", "Categories", nil, nil, "commonmenu", "interaction_bgd")
local show_car = RageUI.CreateSubMenu(mainMenu, "", "Vehicules")
local selection = RageUI.CreateSubMenu(show_car, "", "Interaction")
            ...
            while open do
                ...
                RageUI.IsVisible(mainMenu, function()
                    for _,v in pairs(categories) do
                        RageUI.Button(("~b~→~s~ %s"):format(v.label), nil, {RightBadge = RageUI.BadgeStyle.Star}, true, {
                            onSelected = function()
                                getVehicles(v.name)
                                getStockage(v.name)
                            end
                ...
                RageUI.IsVisible(show_car, function()
                    for _,v in pairs(vehicles) do
                        RageUI.Button(("~b~→~s~ %s"):format(v.name), nil, {RightBadge = RageUI.BadgeStyle.Tick}, true, {
                            onActive = function()
                                RageUI.Info(("~b~%s~s~"):format(v.name),{"Prix:"}, {("~g~%s$~s~"):format(v.price)})
                            end,
                            onSelected = function()
                ...
                RageUI.IsVisible(selection, function()
                    for _, v in pairs(select) do
                        RageUI.Button(("Tester le véhicule (~b~%smin~s~)"):format(xCarDealer.TimeForTest), nil, {RightLabel = "→"}, true, {
                            onActive = function()
                                ...
                                if not car then
                                    ...
                                    entity = CreateVehicle(model, xCarDealer.Position.Exposition.x, xCarDealer.Position.Exposition.y, xCarDealer.Position.Exposition.z, heading, false, false)
                                    
                                    -- Rendre invisible pour les autres joueurs
                                    NetworkSetEntityInvisibleToNetwork(entity, true)  
                                    SetEntityAsMissionEntity(entity, true, true) 
                
                                    -- Verrouiller et protéger
                                    SetVehicleDoorsLocked(entity, 2)  
                                    SetEntityInvincible(entity, true)
                
                                    -- Nettoyage du modèle après usage
                            ...
                            onSelected = function()
                                ...
                                local vehicle = CreateVehicle(model, xCarDealer.Position.SpwanCarForTest.x, xCarDealer.Position.SpwanCarForTest.y, xCarDealer.Position.SpwanCarForTest.z, xCarDealer.Position.HeadingForTest, true, false)
                        ...
                        RageUI.Line()
                        RageUI.Separator(("~b~→~s~ Nombre de place: ~b~%s~s~"):format(GetVehicleMaxNumberOfPassengers(entity) + 1))
                        RageUI.Separator(("~g~→~s~ Prix: ~g~%s$~s~"):format(v.price))
