...
AddEventHandler('esx:playerDropped', function(src, reason)
    ...
    -- Récupération du status du joueur
    local status = xPlayer.get('status')
    if status and next(status) ~= nil then
        -- Mise à jour dans la base de données avec les données encodées en JSON
        MySQL.Async.execute('UPDATE users SET status = ? WHERE identifier = ?', {
